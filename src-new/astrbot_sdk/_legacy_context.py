"""旧版 API 兼容层 — 会话管理与 Context 实现。

这个模块承接旧 ``Context`` / ``ConversationManager`` 的运行时行为，
把仍然可映射到 v4 的能力落到 v4 ``Context`` 客户端上，
无法等价支持的旧接口则显式给出迁移错误，而不是静默降级。

不要从 ``_legacy_star`` 导入任何符号（避免循环依赖）。
外部代码应通过 ``_legacy_api`` 聚合入口导入。
"""
