...
1. <span style="color: gray">~~In the left menu, click Config → System Config.~~ (Archived: since v4.26.0, system configuration has moved to `Settings` at the bottom of the sidebar)</span> Open `Settings` → `Security` → `WebUI Security` at the bottom of the sidebar.
...
- Project workspaces are supported: browse and manage files in the current project workspace from the chat page.
- Text pastes longer than 10,000 characters are automatically converted into text attachments.
- Inline interactive GenUI is supported, allowing plugins to render interactive components in the chat page.
- WebChat accepts drag-and-drop uploads across the whole chat area.
...
In the admin panel, you can configure AstrBot's plugins through visual configuration. Click `Config` in the left sidebar to enter the configuration page.
...
In the admin panel, you can view installed plugins and install new plugins through the `Plugins` section in the left sidebar. The page contains the `Plugins`, `Plugin Market`, `MCP`, `Skills`, and `Handlers` tabs.

Click the `Plugin Market` tab to browse the plugin market powered by AstrBot Cloud (<https://cloud.astrbot.app>). Results can be sorted by downloads, and plugin details show the last update time.

![image](https://files.astrbot.app/docs/source/images/webui/image-1.png)

Click the + button in the bottom right corner to install a plugin using one of the following methods:

- One-click installation from the plugin market.
- A repository URL, including HTTP(S), SSH, and SCP-style Git repositories.
- A local directory, to install a local plugin.
...
<span style="color: gray">~~Use the `Command Management` menu on the left to centrally manage all registered commands.~~ (Archived: since v4.27.0, command management has moved to the `Handlers` tab of the `Plugins` page)</span>

Open the `Plugins` page and switch to the `Handlers` tab to centrally manage all registered commands and function tools; system plugins are hidden by default.
...
Open `Console` in the `More` group to view real-time AstrBot logs. Logs support category filtering, and a `Hide user chat entries` toggle is available to stop showing user message contents in the console.

Each plugin can also have its own log level (`DEBUG`, `INFO`, `WARNING`, `ERROR`, etc.) configured in its details, which makes per-plugin troubleshooting easier.

## Settings

Click `Settings` at the bottom of the sidebar to open the settings page, which contains the following sections:

- `General`: timezone, external callback URL (callback API Base), logs, cache, and other runtime basics.
- `Appearance`: WebUI theme mode — light, dark, or follow system — plus sidebar customization.
- `Network`: runtime proxy, Python dependency install arguments, PyPI index, and the GitHub proxy address.
- `Security`: WebUI security options such as HTTPS, login rate limiting, and TOTP two-factor authentication.
- `Maintenance`: configuration backup and AstrBot restart actions.
- `OpenAPI`: API key and scope management.
- `About`: version info, changelog, and related resource links.

The effective AstrBot time is also shown under `Settings` → `General`, which helps confirm the timezone configuration.

## Personas

On the `Persona` page you can manage personas (Persona) and import or export them as files for sharing and backup. Exported personas exclude tool and Skill configuration.
...
Run one of the following commands in the AstrBot working directory to reset or change the password:

```bash
# Regenerate the initial password and print it in the startup logs
astrbot run --reset-password

# Or change the password interactively
astrbot password
```
...
<span style="color: gray">~~Modify the `password` in the `dashboard` configuration in the data/cmd_config.json file and delete the entire password key-value pair.~~ (Archived: replaced by the CLI commands above; see the FAQ if you need to edit the file manually)</span>
