...
Open `Extensions → Handlers → Function Tools` (`/extension/components`) in the WebUI to view and manage tool enablement. Configure the tools available to each Persona on the `Persona` page, and manage MCP servers under `Extensions → MCP Servers`. Per-tool permissions can also be configured there.
