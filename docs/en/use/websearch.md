...
Open `Config`, select the profile to edit, and enable Web Search under `AI → Capabilities → Web Search`, where you can select `Tavily`, `BoCha`, `Baidu AI Search`, `Brave`, `Firecrawl`, `Exa`, `AnySearch`, or `Serply`. Fill in the required settings and click `Save Configuration` at the bottom right.
