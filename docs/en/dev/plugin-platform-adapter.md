...
class FakePlatformAdapter(Platform):
    ...
    async def run(self):
        ...
        )  # Continuously listens for messages; this is a blocking call.
    ...
    async def convert_message(self, data: dict) -> AstrBotMessage:
        ...
        )  # Also friend_message for private chats. Analyze per platform. Important!
        ...
        ]  # Message chain. Append other message types as needed. Important!
...
class FakePlatformEvent(AstrMessageEvent):
    ...
    async def send(self, message: MessageChain):
        ...
        await super().send(
            message
        )  # Must be called at the end to invoke the parent class's send method.
```
