...
| Voice | Yes | Yes | |
| Video | Yes | Yes | |
| File | Yes | Yes | Large local files are sent via chunked upload (>= v4.27.3) |

Proactive message push: Supported. In Webhook mode, proactive group messages can be sent even without a cached passive-reply `msg_id` (>= v4.26.0).
...
3. Under `Choose setup method`, select `One-click QR setup`, click start, customize the bot `ID` in the dialog if needed (it is auto-generated by default), then scan the QR code with mobile QQ.
