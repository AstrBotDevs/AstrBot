...
4. After you confirm the QR binding, AstrBot automatically fills in `AppID` and `AppSecret`. Make sure `Enable` is checked, keep `Unified Webhook Mode` enabled, then click `Save`.
