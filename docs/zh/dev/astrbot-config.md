...
此旧配置项现为 `agent_runner.config.misc.max_steps`，限制每次用户请求中 Agent 主循环的最大执行轮数，默认为 `128`。同一轮模型生成可以调用多个工具，这些调用合计只算一步。

上限至少为 16 步时，在已用预算达到 80%、90%、95%（向上取整）时各提醒一次，将剩余步数与收尾提示追加到当前轮最后一条工具结果。上下文压缩不会重置预算或提醒状态。达到上限后会禁用工具，并额外允许一次最终回复。用户可以直接要求继续执行，获得新的步数预算，无需修改配置；也可以提高上限，让单次请求执行更多轮。

升级到配置版本 4 时，已有本地 Agent 的 `30` 步上限会一次性调整为 `128`。其他值保持不变，之后手动修改的值（包括改回 `30`）都会保留。
