...
| 语音 | 是 | 是 | |
| 视频 | 是 | 是 | |
| 文件 | 是 | 是 | 本地大文件通过分片上传（>= v4.27.3） |

主动消息推送：支持。即使没有缓存的被动回复 `msg_id`，Webhook 模式也支持向群聊主动推送消息（>= v4.26.0）。
...
3. 在 `选择创建方式` 中选择 `扫码一键创建`，点击开始创建后，可以在弹窗中自定义机器人 `ID`（默认自动生成），然后用手机 QQ 扫描页面中的二维码。
4. 扫码确认后，点击 `保存`。
