...
| --- | --- | --- | --- |
| 文本 | 是 | 是 | |
| 图片 | 是 | 是 | 支持 photo album |
| 语音 | 是 | 是 | |
| 视频 | 是 | 是 | 支持 video/photo 混合 album |
| 文件 | 是 | 是 | 连续 `File` 会作为 document album 发送 |
...
Telegram 适配器会自动把连续的 `Image`、`Video`、`File` 发送为 Telegram media group：

- `Image` 会作为 photo 发送，`Video` 会作为 video 发送，二者可以混合成同一个视觉 album。
- `File` 会作为 document 发送，连续 `File` 会组成 document album。
- `Record` 仍然表示 voice message，不会被映射成 audio album。
- 相邻 `Plain` 会作为媒体或 album 的 caption；caption 按 Telegram Bot API 限制最多 1024 字符，超出会报错。
- Telegram media group 不支持 `reply_markup`。需要按钮时，请把 Inline Keyboard 放到单独消息里。
- 需要 Telegram audio album 或 spoiler、视频 streaming、缩略图等常用高级媒体参数时，请使用开发指南中的 `TelegramMediaGroup`。
...
- `telegram_proxy`：仅用于此 Telegram 适配器的 Bot API 请求，也会用于 `getUpdates` 轮询请求。
...
- `telegram_command_registered_plugins`：插件选择器。默认值为 `["*"]`，表示注册全部已启用插件的指令；选择具体插件后，只注册这些插件提供的指令；选择“不启用任何插件”时保存为 `[]`，会清空 Telegram 端已注册的插件指令。若插件在插件页面被禁用，此处的选择不会生效。
- `telegram_command_scopes`：注册范围模板列表。默认值为 `[{"__template_key": "default", "type": "default"}]`。
...
```json
[
  {"__template_key": "default", "type": "default", "language_code": "zh"},
  {"__template_key": "chat", "type": "chat", "chat_id": 123456789}
]
```

直接编辑配置文件时，`__template_key` 只用于面板选择模板；运行时也兼容旧格式，例如 `{"type": "chat", "chat_id": 123456789}`。
