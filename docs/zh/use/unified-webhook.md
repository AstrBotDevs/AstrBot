...
4. 进入左侧边栏底部的 `设置` → `常规` → `基础运行`，将 `对外可达的回调接口地址` 填写为配置的 URL 地址（如 https://astrbot.example.com），点击保存，等待重启。<span style="color: gray">~~前往 AstrBot `配置文件` 页，点击 `系统`，将 `对外可达的回调接口地址` 填写为配置的 URL 地址。~~（已归档：自 v4.26.0 起，系统配置迁移至 `设置` 页面）</span>
