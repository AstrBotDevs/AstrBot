...
如果你的部署环境无法使用插件面板，也可以手动安装到 `data/plugins`。这种方式不推荐作为首选，因为后续更新、启停和状态查看都没有插件面板顺手。
...
- **占用租约**：占用的有效时间。默认 600 秒。Agent 可以调用续租工具延长有效租约；续租后，后续普通工具调用不会把更长的租约缩短。租约过期后，当前会话不再绑定该沙盒；Agent 需要先查看沙盒列表，再切换、接管或创建沙盒后继续工作。
...
在配置 `CUA` 之前，推荐先在 WebUI 的“插件管理”页面点击右下角 `+`，通过 URL 安装插件：

```text
https://github.com/AstrBotDevs/astrbot_sandbox_cua
```

如果无法使用插件面板，再手动安装到 `data/plugins`：
...
在 AstrBot 侧配置 `Shipyard Neo` 之前，推荐先在 WebUI 的“插件管理”页面点击右下角 `+`，通过 URL 安装插件：
...
在配置 `BoxLite` 之前，推荐先在 WebUI 的“插件管理”页面点击右下角 `+`，通过 URL 安装 `BoxLite` 插件。当前 `BoxLite` 还依赖旧版 `Shipyard` 插件中的部分代码，因此也需要安装 `Shipyard` 插件。
