...
在 AstrBot 中，默认提供了网页搜索、待办提醒、`send_message_to_user`（发送多媒体消息给用户）、`get_group_message_history`（搜索当前群聊的持久化历史消息，v4.27.0 起）等工具。<span style="color: gray">~~默认提供了网页搜索、待办提醒、代码执行器这些工具~~（已归档：代码执行器已由电脑能力与 Agent 沙盒环境取代）</span>很多插件，如:
...
打开 WebUI 的 `插件 → 管理行为 → 函数工具`（`/extension/components`）查看和管理工具的启用状态，也可为每个函数工具单独配置使用权限。人格可使用的工具范围在 `人格设定` 中配置；MCP 服务器在 `插件 → MCP` 中管理。
