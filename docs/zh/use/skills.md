...
> [!NOTE]
> 需要说明的是，如果您使用 Local 作为执行环境，Shell、Python 和本机文件写入分别按 `tool.local_exec`、`tool.python_exec` 和 `tool.file_write` 授权。这些高风险动作只允许 Dashboard 控制面在 step-up 后执行；IM、插件、Agent 和 API Key 会被拒绝。普通用户会收到权限限制提示，例如 `Sorry, I cannot execute code on your local environment due to permission restrictions.`。
