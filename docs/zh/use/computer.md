...
在本地权限表中，展开“文件访问范围”下拉菜单即可查看各选项的说明。“工作区”允许受限访问当前工作区、平台附件和技能文件；“整个环境”（原“主机文件”）允许访问 AstrBot 运行账户有权限访问的文件。Docker 部署时，“整个环境”指容器内及挂载的文件，并非宿主机全部文件。“关闭”不允许通过本地电脑能力工具访问任何文件。范围说明已从表格下方移到下拉菜单的对应选项中；将鼠标悬停在“AstrBot 所在运行环境的所有文件。”后的问号上，或点按问号，可查看账户权限和 Docker 访问范围的详细说明。

本地模式按身份分别配置 `filesystem_scope`、代码执行和联网权限。`none` 禁用本地文件工具，`host` 允许访问运行账户有权限访问的文件。`workspace` 使用以下目录规则：

| 目录 | 普通成员 | 管理员 |
| --- | --- | --- |
| 当前会话或项目工作区 | 读写 | 读写 |
| `data/skills` | 只读 | 读写 |
| `data/plugins/*/skills`、`astrbot/builtin_stars/*/skills` | 只读 | 只读 |
| `data/temp/platform_files/<normalized_umo>/`（当前会话） | 只读 | 读写 |
| 其他会话的 `platform_files` 子目录 | 不可访问 | 读写 |
| 共享 `data/temp` 和系统临时目录 `.astrbot` 中的其他文件 | 不整体授权 | 不整体授权 |

平台附件在会话标识确定后的公共预处理阶段归档，覆盖文件、图片、语音、视频和引用／转发中的附件。目录名复用工作区的 UMO 规范化规则；群聊独立会话使用隔离后的最终 UMO。适配器或公共下载器可能先写入内部中转文件，再归档到对应会话目录。原始附件对成员只读，需要编辑时请复制到当前工作区。归档目录仍受临时目录容量清理约束，不是永久存储。

read/Grep、write/edit、受限 Shell/Python 和发送文件工具使用对应的会话边界。生成文件、沙箱下载文件和本地工具长输出放在当前工作区，不需要开放整个临时目录。既有散落临时文件不会自动迁移，因为它们没有可靠的会话归属；旧消息重新经过预处理时会归档其可用附件。

macOS/Linux 新配置默认让成员和管理员使用 `workspace`，成员默认不能执行代码；Windows 默认关闭成员访问，管理员使用 `host`。旧配置迁移可能保留管理员的整机访问权限。`computer_use_require_admin` 继续用于第三方沙箱操作，不能替代本地按身份配置的目录权限。
