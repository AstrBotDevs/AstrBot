...
- 私聊中默认普通用户可使用。
- 群聊默认需要 AstrBot 管理员权限，与是否开启会话隔离无关；管理员指配置的管理员 ID，不是自动识别的群管理员。
- 在 WebUI「管理行为 → 指令」中，可以分别调整 `new` 和 `reset` 的权限：「所有人」「仅管理员」「仅群聊限管理员」。设置对所有配置文件生效，已有的显式权限设置优先于命令默认权限。
- 默认选择「仅群聊限管理员」；选择「所有人」即可允许普通群成员使用。关闭「隔离对话」时影响整个群，开启时只影响自己的对话。
- 指令的禁用、重命名仍由指令管理统一处理。

升级提示：旧的 reset 场景权限设置不再使用。未单独配置指令权限时，普通群成员需要管理员在指令管理中将对应指令设为「所有人」才能新建对话。「隔离对话」的配置保持不变。
