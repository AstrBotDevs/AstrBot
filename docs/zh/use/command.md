...
- 在 WebUI「平台配置 → 基本」开启「允许非管理员在群聊中新建对话」后，普通成员可以使用。共享上下文时影响整个群，成员隔离时只影响自己的对话。
- 指令管理中的额外权限限制仍然有效；该开关不会绕过指令的禁用或管理员限制。

升级提示：旧的 reset 场景权限设置不再使用。新开关默认关闭，即使原来开启了会话隔离，升级后也需要显式开启它才能允许普通群成员新建对话。隔离会话本身的配置不变。
