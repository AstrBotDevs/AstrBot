...
进入 AstrBot 管理面板，点击左栏 **模型提供商 → 新增提供商 → OpenAI**。
...
| 能力 | AstrBot 配置位置 |
|------|-----------------|
| 语音转文字 (STT) | 模型提供商 → 语音转文字 |
| 文字转语音 (TTS) | 模型提供商 → 文字转语音 |
| 嵌入 (Embedding) | 模型提供商 → 嵌入 |
| 重排序 (Rerank) | 模型提供商 → 重排序 |

所有能力使用同一个 API Key 和 API Base，无需额外配置。

## 设为默认

前往 **配置文件 → 提供商设置**，将「默认聊天模型提供商」改为刚创建的 AIHubMix 提供商，保存即可。
