...
默认 `User-Agent` 为 `astrbot/<版本>`。非空白值的优先级为：单次请求的 `extra_headers` → 提供商的 `custom_headers` → 默认值。User-Agent 键名不区分大小写，空白值回退到下一层。

`x-opencode-session` 根据对话 ID 自动生成，不允许手动覆盖。未传入对话 ID 的直接调用使用独立会话标识。
