...
2. 在 `配置文件 -> 平台配置` 中，找到 `管理员 ID`，添加你的用户 ID（可以通过 `/sid` 指令获取）。
3. 右下角保存配置
...
1. `/name, /provider, /dashboard_update, /op, /deop, /persona, /llm, /plugin, /model, /groupnew` 等是默认的管理员指令。可以通过 `/sid` 指令得到用户的 ID，然后在 `配置文件` -> `平台配置` 中添加到管理员 ID 名单中。
