...
- [x] **Step 1: pnpm add（在 worktree 的 dashboard 目录）**
...
- [x] **Step 1: 写失败测试 `dashboard/src/utils/codemirrorLanguages.spec.ts`**
...
- [x] **Step 3: 实现 `dashboard/src/utils/codemirrorLanguages.ts`**
...
- [x] **Step 1: 写失败测试 `dashboard/src/components/chat/message_list_comps/CodeMirrorEditor.spec.ts`**
...
- [x] **Step 3: 实现 `dashboard/src/components/chat/message_list_comps/CodeMirrorEditor.vue`**
...
- [x] **Step 3: 切换 `GitIgnoreEditor.vue`（精确改动 5 处）**
...
- [x] **Step 5: 手测清单（dev server + 真实浏览器）**
