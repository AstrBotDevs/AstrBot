...
class ChatService(BaseService):
    ...
    async def get_attachment(self, attachment_id: str | None = None):
        """获取附件文件
        ...
        if attachment_id is None:
            # 从路径参数获取
            attachment_id = (
                request.view_args.get("attachment_id") if request.view_args else None
            )
    ...
    async def chat(self):
        ...
        Body:
        - message: 消息内容（文本或消息段列表）(必填)
        - session_id 或 conversation_id: 会话ID (必填)
        - selected_provider: 选中的提供商 (可选)
        - selected_model: 选中的模型 (可选)
        - enable_streaming: 是否启用流式响应 (可选, 默认 True)

        Returns:
            QuartResponse: SSE 流式响应
        """
    ...
    async def delete_session(self, session_id: str | None = None):
        ...
        if session_id is None:
            # 从路径参数获取
            session_id = (
                request.view_args.get("session_id") if request.view_args else None
            )
        if not session_id:
            # 从查询参数获取
    ...
    async def new_session(self):
        ...
        Query 参数或 Body:
        - platform_id: 平台ID (可选, 默认 webchat)
        """
        username = g.get("username", "guest")
        # 优先从 query 参数获取，如果没有则从 body 获取
        platform_id = request.args.get("platform_id")
        if not platform_id:
            try:
                post_data = await request.json
                platform_id = post_data.get("platform_id", "webchat")
            except Exception:
                platform_id = "webchat"
        # 创建新会话
    ...
    async def update_session_display_name(self, session_id: str | None = None):
        """更新会话显示名称
        ...
        if not session_id:
            ...
            # 如果从路径参数获取了 session_id，再从 body 获取 display_name
            post_data = await request.json
            display_name = post_data.get("display_name")
