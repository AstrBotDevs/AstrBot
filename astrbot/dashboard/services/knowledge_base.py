...
class KnowledgeBaseService(BaseService):
    ...
    async def get_kb(self, kb_id: str | None = None):
        """获取知识库详情
        ...
        try:
            ...
            if kb_id is None:
                # 从路径参数获取
                kb_id = request.view_args.get("kb_id") if request.view_args else None
            if not kb_id:
                # 从查询参数获取
    ...
    async def update_kb(self, kb_id: str | None = None):
        """更新知识库
        ...
        try:
            ...
            if not kb_id:
                ...
                # 如果从路径参数获取了 kb_id，再从 body 获取其他参数
                data = await request.json
    ...
    async def delete_kb(self, kb_id: str | None = None):
        """删除知识库
    ...
    async def list_documents(self, kb_id: str | None = None):
        """获取文档列表
    ...
    async def upload_document(self, kb_id: str | None = None):
        """上传文档
        ...
        路径参数或 Form Data (multipart/form-data):
        - kb_id: 知识库 ID (必填)
        - file: 文件对象 (必填，可多个，字段名为 file, file1, file2, ... 或 files[])

        路径参数或 JSON Body (application/json):
        - kb_id: 知识库 ID (必填)
        - files: 文件数组 (必填)
          - file_name: 文件名 (必填)
    ...
    async def import_documents(self, kb_id: str | None = None):
        """导入预切片文档
        ...
        try:
            ...
            # 获取知识库 (kb_id 已经通过 _validate_import_request 验证，不会是 None)
            assert kb_id is not None, "kb_id should not be None after validation"
            kb_helper = await kb_manager.get_kb(kb_id)
            if not kb_helper:
                return Response().error("知识库不存在").__dict__
    ...
    async def get_upload_progress(
        self, kb_id: str | None = None, task_id: str | None = None
    ):
        """获取上传进度和结果
        ...
        try:
            ...
            if task_id is None:
                # 从路径参数获取
                task_id = (
                    request.view_args.get("task_id") if request.view_args else None
                )
            if not task_id:
                # 从查询参数获取
    ...
    async def get_document(self, kb_id: str | None = None, doc_id: str | None = None):
        """获取文档详情
    ...
    async def delete_document(
        self, kb_id: str | None = None, doc_id: str | None = None
    ):
        """删除文档
    ...
    async def delete_chunk(
        ...
        try:
            ...
            if chunk_id is None:
                # 从路径参数获取
                chunk_id = (
                    request.view_args.get("chunk_id") if request.view_args else None
                )

            if not kb_id or not doc_id or not chunk_id:
                # 从 body 获取
                data = await request.json
                if not kb_id:
                    kb_id = data.get("kb_id")
    ...
    async def list_chunks(self, kb_id: str | None = None, doc_id: str | None = None):
        """获取块列表
    ...
    async def upload_document_from_url(self, kb_id: str | None = None):
        """从 URL 上传文档
