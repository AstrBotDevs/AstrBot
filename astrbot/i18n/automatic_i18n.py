...
def extract_and_rewrite(source: str):
    ...
    for node in ast.walk(tree):
        ...
        # extra 参数，最终 end_pos = 最后一个参数文本的结尾
        if extra_arg_nodes:
            # 找到第一个参数后面的逗号位置作为搜索起点
            search_from = arg_end
            last_end_pos = arg_end
            found_all = True
            for extra_node in extra_arg_nodes:
                extra_seg = ast.get_source_segment(source, extra_node)
                ...
                if comma == -1:
                    found_all = False
                    break
                found = source.find(extra_seg, comma + 1)
                if found == -1:
                    found_all = False
                    break
