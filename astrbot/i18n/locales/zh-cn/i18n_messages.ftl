...
msg-b3665aee = 区域设置目录不存在：{ $base_dir }
msg-3fe89e6a = 未找到区域设置目录位于：{ $base_dir }
msg-c79b2c75 = 命名空间 '{ $namespace }已存在。设置 replace=True 以覆盖。
msg-7db3fccf = 默认命名空间无法注销。
msg-3d066f64 = 命名空间 '{ $namespace }未注册。

### astrbot/core/persona_mgr.py

msg-51a854e6 = 已加载{ $res }Personality.
msg-1ea88f45 = ID 为 {$id} 的角色{ $persona_id }不存在。
msg-28104dff = ID为{$id}的角色{ $persona_id }已存在。
msg-08ecfd42 = { $res }The format for the personality scenario preset dialogue is incorrect; the number of entries must be even.
...
msg-ccc87294 = 正在从指定分支{ $branch }下载{ $author }待翻译文本：{ $repo }
msg-dfebcdc6 = 获取{ $author }待翻译文本：{ $repo }的 GitHub Releases 失败:{ $e }，将尝试下载默认分支
msg-e327bc14 = 正在从默认分支下载{ $author }待翻译文本：{ $repo }
msg-3cd3adfb = 检查到设置了镜像站，将使用镜像站下载{ $author }待翻译文本：{ $repo }仓库源码:{ $release_url }
...
msg-29e3b482 = 子代理角色 %s 未找到，回退到内联提示。
msg-f425c9f0 = 已注册的子代理转交工具：{ $res }

### astrbot/core/astr_main_agent.py

msg-8dcf5caa = 未找到指定的提供商: %s。
msg-61d46ee5 = 选择的提供商类型无效(%s)，跳过 LLM 请求处理。
msg-496864bc = 选择提供商时发生错误：%s
msg-507853eb = 无法创建新的对话。
msg-66870b7e = 检索知识库时发生错误：%s
msg-36dc1409 = 文件提取的Moonshot AI API密钥未设置
msg-8534047e = 不支持的文件提取提供程序：%s
msg-f2ea29f4 = 无法获取图片描述，因为提供商 `{ $provider_id }不存在。
msg-91a70615 = 无法获取图片标题，因为提供者 `{ $provider_id }` 不是有效的 Provider，它是{ $res }.
msg-b1840df0 = 正在使用提供程序处理图片说明：%s
msg-089421fc = 处理图片描述失败: %s
msg-719d5e4d = 引用中未找到图像说明提供程序。
msg-e16a974b = 处理引用图片失败: %s
msg-037dad2e = 组名显示已启用但组对象为空。组ID：%s
msg-58b47bcd = 时区设置错误: %s, 使用本地时区
msg-938af433 = 提供者 %s 不支持图片，使用占位符。
msg-83d739f8 = 提供商 %s 不支持工具使用，正在清除工具。
msg-3dbad2d9 = sanitize_context_by_modalities 已应用：已移除图像块=%s，已移除工具消息=%s，已移除工具调用=%s
msg-4214b760 = 为会话 %s 生成的聊天界面标题：%s
msg-cb6db56e = 不支持的llm_safety_mode策略：%s。
...
msg-614aebad = 备用聊天提供商 `%s` 未找到，已跳过。
msg-1a2e87dd = 备用聊天提供商 `%s` 的类型无效：%s，跳过。
msg-ee979399 = 未找到任何对话模型（提供商），跳过 LLM 请求处理。
msg-7a7b4529 = 由于限制=%d 对于umo=%s，跳过引用的备用图像
msg-46bcda31 = 截断引用回退图片，用户消息对象=%s，回复ID=%s，从 %d 到 %d
msg-cbceb923 = 无法解析回退引用图片，用户消息对象=%s，回复ID=%s：%s
msg-31483e80 = 应用文件提取时发生错误：%s

### astrbot/core/umop_config_router.py

msg-dedcfded = umop 键必须是格式为 [platform_id]:[message_type]:[session_id] 的字符串，其中可选通配符 * 或留空表示全部
msg-8e3a16f3 = umop 必须是格式为 [platform_id]:[message_type]:[session_id] 的字符串，其中可选通配符 * 或留空表示全部

### astrbot/core/event_bus.py

msg-da466871 = 找不到ID为以下值的PipelineScheduler：{ $res }，事件被忽略。
msg-7eccffa5 = { $conf_name }] [{ $res }待翻译文本：{ $res_2 })]{ $res_3 }/{ $res_4 }待翻译文本：{ $res_5 }
msg-88bc26f2 = [{ $conf_name }] [{ $res }待翻译文本：{ $res_2 })]{ $res_3 }:{ $res_4 }

### astrbot/core/astr_agent_tool_exec.py

msg-e5f2fb34 = 后台任务{ $task_id }失败：{ $e }
msg-c54b2335 = 后台交接{ $task_id }待翻译文本：{ $res }) 失败：{ $e }
msg-8c2fe51d = 构建后台任务主代理失败{ $tool_name }.
msg-c6d4e4a6 = 后台任务代理未收到响应
msg-0b3711f1 = 必须为本地函数工具提供事件。
msg-8c19e27a = 工具必须有一个有效的处理器或重写“运行”方法。
msg-24053a5f = 工具直接发送消息失败:{ $e }
msg-f940b51e = 工具{ $res }执行超时后{ $res_2 }秒。
msg-7e22fc8e = 未知的方法名：{ $method_name }
msg-c285315c = 工具执行值错误：{ $e }
msg-41366b74 = 工具处理器参数不匹配，请检查处理器定义。处理器参数：{ $handler_param_str }
msg-e8cadf8e = 工具执行错误：{ $e }. 回溯追踪：{ $trace_ }
msg-d7b4aa84 = 上一个错误：{ $trace_ }

### astrbot/core/astr_agent_run_util.py

msg-6b326889 = 代理达到最大步数限制（{ $max_step })，强制最终响应。
msg-bb15e9c7 = { $status_msg }
msg-78b9c276 = { $res }
msg-9c246298 = on_agent_done 钩子中的错误
...
msg-5ace3d96 = [实时客服信息推送] 分句:{ $temp_buffer }
msg-bc1826ea = [在线客服信息推送器] 错误：{ $e }
...
msg-57dcc41f = 对话 ID{ $cid }未找到

### astrbot/core/updator.py

msg-e3d42a3b = 正在终止{ $res }子进程。
msg-e7edc4a4 = 正在终止子进程{ $res }
msg-37bea42d = Child process{ $res }没有被正常终止, 正在强行杀死。
msg-cc6d9588 = Restart Failed{ $executable },{ $e }），请尝试手动重启。
msg-0e4439d8 = Does not support updating AstrBot started in this manner.
...
msg-9967ec8b = 正在使用代理：{ $proxy_config }
msg-5a29b73d = HTTP 代理已清除
msg-fafb87ce = 子代理编排器初始化失败：{ $e }
msg-f7861f86 = AstrBot 迁移失败：{ $e }
msg-78b9c276 = { $res }
msg-967606fd = 任务{ $res }发生错误:{ $e }
msg-a2cd77f3 = 待翻译文本：{ $line }
msg-1f686eeb = 待翻译文本：
msg-9556d279 = AstrBot 启动完成。
msg-daaf690b = 钩子(on_astrbot_已加载) ->{ $res }-{ $res_2 }
msg-4719cb33 = 插件{ $res }Not properly terminated{ $e }, 可能会导致资源泄露等问题。
...
msg-c240d574 = 阶段{ $res }Event propagation has been terminated.
...
msg-df881b01 = 已捕获活动代理运行的后续消息，用户消息对象=%s，订单序列=%s
...
msg-60493581 = 不支持的工具模式：%s，回退到类似技能模式
msg-9cdb2b6e = 跳过 LLM 请求：消息为空且无 provider_request
msg-e461e5af = 准备请求LLM提供商
msg-be33dd11 = 后续工单已被消耗，停止处理。umo=%s, seq=%s
msg-abd5ccbc = 已为LLM请求获取会话锁
msg-f9d617d7 = 由于安全原因，提供者API基础%s已被阻止。请使用其他AI提供者。
msg-3247374d = [内部代理] 检测到 Live Mode，启用 TTS 处理
...
msg-78b9c276 = { $res }
msg-add19f94 = { $res }-{ $res_2 }Stopped event propagation.
msg-813a44bb = Streaming output enabled, skipping result decoration stage.
msg-891aa43a = Segment reply regex error, using default segmentation method:{ $res }
msg-82bb9025 = Session{ $res }未配置文本转语音模型。
...
msg-c733275f = 您的消息或大模型的回复中包含不适当的内容，已被屏蔽。
...
msg-7b9074fa = { $platform }Pre-reply emoji sending failed:{ $e }
...
msg-2d7497a5 = 检查到配置项{ $path }Child item order inconsistent, reordered.
msg-5fdad937 = 检查到配置项顺序不一致，已重新排序
msg-555373b0 = 未找到 Key: '{ $key }待翻译文本：
...
msg-b593f13f = 无法转换消息类型{ $res }回退到FRIEND_MESSAGE。
msg-98bb33b7 = 清除{ $res }Additional information:{ $res_2 }
msg-0def44e2 = { $result }
msg-8e7dc862 = { $text }
...
msg-18f0e1fe = Loading platform adapters{ $res }Failed, reason:{ $e }待翻译文本：
msg-2636a882 = 未找到适用于{ $res }待翻译文本：{ $res_2 }) 平台适配器，请检查是否已经安装或者名称填写错误
msg-c4a38b85 = 钩子（平台加载后） ->{ $res }-{ $res_2 }
msg-967606fd = 任务{ $res }发生错误:{ $e }
msg-a2cd77f3 = |{ $line }
msg-1f686eeb = -------
...
msg-c81e728d = 2
msg-d6371313 = 钉钉：{ $res }
msg-a1c8b5b1 = The DingTalk private chat session lacks staff_id mapping, falling back to using session_id as userId for sending.
...
msg-5ab2d58d = Failed to clean temporary files:{ $file_path },{ $e }
...
msg-be63618a = The DingTalk adapter has been disabled.
...
msg-ad355b59 = { $signed }
msg-1f6260e4 = _解析器未知事件 %s。
msg-cef08b17 = 将在{ $res }待翻译文本：{ $res_2 }端口启动 QQ 官方机器人 webhook 适配器。

### astrbot/core/platform/sources/qqofficial_webhook/qo_webhook_adapter.py

msg-3803e307 = [QQOfficialWebhook] 会话 %s 没有缓存的 msg_id，跳过 send_by_session
msg-08fd28cf = [QQOfficialWebhook] 通过会话发送消息时遇到不支持的消息类型：%s
...
msg-57c802ef = [Discord] Image 组件缺少 file 属性:{ $i }
...
msg-41dd4b8f = [Discord] 原始 Base64 解码失败，已作为本地路径处理：{ $file_content }
...
msg-c9601653 = Bot 未就绪：self.user 为 None
...
msg-22add467 = [Discord] 斜杠命令 '{ $cmd_name }被触发。 原始参数:{ $params }'. 构建的指令字符串: '{ $message_str_for_filter }待翻译文本：
...
msg-93cbb9fa = { $err }
msg-a4487a03 = Failed to reply to message:{ $e }
msg-48bc7bff = 猜测拉格朗日
msg-6ab145a1 = 获取文件失败:{ $ret }
msg-457454d7 = Failed to retrieve file:{ $e }此消息段将被忽略。
...
msg-eefbe737 = [Lark] API 客户端 im 模块未初始化
...
msg-ef90038b = Cannot upload Lark image ({ $res }):{ $res_2 }
msg-d2065832 = [Lark] 图片上传成功但未返回数据(data is None)
msg-dbb635c2 = { $image_key }
msg-d4810504 = [Lark] 检测到文件组件，将单独发送
msg-45556717 = [Lark] 检测到音频组件，将单独发送
msg-959070b5 = [Lark] 检测到视频组件，将单独发送
msg-4e2aa152 = Feishu does not currently support message segments.{ $res }
...
msg-236bcaad = [Lark] 下载消息资源失败 type={ $resource_type }, 键={ $file_key }, 代码={ $res }，消息={ $res_2 }
...
msg-e5b8829d = 企业微信语音上传返回:{ $response }
...
msg-8fd19bd9 = Failed to fetch WeChat customer service, open_kfid is empty.
...
msg-f8694a8a = Verification request validity succeeded.
msg-8f4cda74 = 验证请求有效性失败，签名异常，请检查配置。
...
msg-bed995d9 = 重试窗口缓冲区命中：用户={ $from_user }
msg-3a94b6ab = 被动窗口中的微信已完成消息发送：用户={ $from_user }消息ID={ $msg_id }
msg-50c4b253 = 微信在被动窗口完成消息发送但非最终状态：用户={ $from_user }消息ID={ $msg_id }
msg-7d8b62e7 = wx在窗口中完成但非最终状态；返回占位符：user={ $from_user }消息ID={ $msg_id }
msg-2b9b8aed = 在被动窗口中微信任务失败
msg-7bdf4941 = 微信被动窗口超时时间：用户={ $from_user }消息ID={ $msg_id }
msg-98489949 = wx 触发思考时：用户={ $from_user }
msg-01d0bbeb = wx new trigger: user={ $from_user }消息ID={ $msg_id }
msg-52bb36cd = wx 开始任务：用户={ $from_user }消息标识符={ $msg_id }preview={ $preview }
msg-ec9fd2ed = wx buffer 立即命中: user={ $from_user }
msg-61c91fb9 = 第一个窗口中wx未完成；返回占位符：user={ $from_user }msg_id={ $msg_id }
msg-35604bba = 首窗口微信任务失败
msg-e56c4a28 = wx 第一个窗口超时：用户={ $from_user }消息ID={ $msg_id }
msg-e163be40 = 将在{ $res }待翻译文本：{ $res_2 }端口启动 微信公众平台 适配器。
...
msg-a10a224d = [Misskey] 收到通知事件: type={ $notification_type }用户ID={ $res }
...
msg-ea465183 = [Misskey] 收到未处理事件: type={ $event_type }，频道={ $res }
...
msg-af584ae8 = [Misskey] 解析可见性: visibility={ $visibility }, visible_user_ids={ $visible_user_ids }，session_id={ $session_id }用户ID用于缓存={ $user_id_for_cache }
msg-1a176905 = [Misskey] 发送消息失败：{ $e }

### astrbot/core/platform/sources/misskey/misskey_api.py

msg-fab20f57 = Misskey API 需要 aiohttp 和 websockets 库。请通过以下命令安装：pip install aiohttp websockets
...
msg-7de2ca49 = [Misskey API]{ $func_name }第{ $attempt }Secondary retry failure:{ $e }，{ $sleep_time }s后重试
...
msg-2bea8c2e = [Misskey API] 服务器内部错误：{ $endpoint }待翻译文本：(HTTP{ $status })
...
msg-dc96bbb8 = HTTP{ $status }为{ $endpoint }
msg-4c7598b6 = [Misskey API] 已获取到{ $res }1 new notification
...
msg-cd43861a = [Misskey API] find 响应: 找到{ $res }1 file
msg-05cd55ef = [Misskey API] 根据名称查找文件失败:{ $e }
msg-c01052a4 = [Misskey API] 列表文件请求: limit={ $limit }文件夹ID={ $folder_id }, type={ $type }
msg-7c81620d = [Misskey API] 列表文件响应：已找到{ $res }个文件
...
msg-71cc9d61 = [Misskey API] URL媒体处理失败{ $url }待翻译文本：{ $e }
...
msg-9fdbafe9 = 无法为stream_id找到对应的回退队列：{ $stream_id }
msg-7a52ca2b = stream_id 对应的待处理队列中没有新消息。{ $stream_id }
...
msg-adaee66c = URL validation parameters missing
msg-742e0b43 = Received enterprise WeChat smart robot WebHook URL verification request.
msg-f86c030c = 消息回调参数缺失
msg-cce4e44c = 收到消息回调，msg_signature={ $msg_signature }时间戳={ $timestamp }, nonce={ $nonce }
...
msg-c55856d6 = 处理会话{ $session_id }Error occurred while messaging:{ $e }
...
msg-06e3f874 = [LINE] %s 消息发送失败：状态码=%s 响应体=%s
msg-1478c917 = [LINE] %s 消息请求失败: %s
msg-39941f06 = [LINE] 获取内容重试失败：消息ID=%s 状态=%s 正文=%s
msg-1fe70511 = [LINE] 获取内容失败：message_id=%s status=%s body=%s

### astrbot/core/platform/sources/line/line_event.py

msg-4068a191 = [LINE] 解析图片URL失败: %s
msg-2233b256 = [LINE] 解析记录URL失败：%s
msg-a7455817 = [LINE] 解析记录时长失败：%s
msg-9d0fee66 = [LINE] 解析视频链接失败：%s
msg-3b8ea946 = [LINE] 解析视频封面失败：%s
msg-aea2081a = [LINE] 生成视频预览失败：%s
msg-af426b7e = [LINE] 解析文件URL失败：%s
msg-fe44c12d = [LINE] 解析文件大小失败：%s
...
msg-64e92929 = [LINE] 无效的Webhook签名
msg-71bc0b77 = [LINE] webhook 内容无效：%s
msg-8c7d9bab = [LINE] 重复事件已跳过: %s

### astrbot/core/platform/sources/telegram/tg_event.py

msg-7757f090 = [Telegram] 发送聊天动作失败：{ $e }
msg-80b075a3 = 用户隐私设置阻止接收语音消息，已回退为发送音频文件。要启用语音消息功能，请前往 Telegram 设置 → 隐私与安全 → 语音消息 → 设置为“所有人”。
...
msg-908d0414 = An error occurred while registering the command to Telegram:{ $e }
msg-d2dfe45e = Command name{ $cmd_name }重复注册，将使用首次注册的定义：{ $res }待翻译文本：
msg-63bdfab8 = 收到启动命令但没有有效的聊天会话，跳过 /start 回复。
msg-03a27b01 = Telegram 消息：{ $res }
msg-e47b4bb4 = 收到一条没有消息内容的更新。
msg-c97401c6 = [Telegram] 收到一条未包含发件人信息的消息。
msg-f5c839ee = Telegram文档文件路径为空，无法保存文件。{ $file_name }.
msg-dca991a9 = Telegram 视频文件路径为空，无法保存文件。{ $file_name }待翻译文本：
msg-56fb2950 = 创建媒体组缓存：{ $media_group_id }
msg-0de2d4b5 = 向媒体组添加消息{ $media_group_id }当前拥有{ $res }items.
msg-9e5069e9 = 媒体组{ $media_group_id }已达到最大等待时间（{ $elapsed }s >={ $res }s), 立即处理。
msg-9156b9d6 = 已安排的媒体组{ $media_group_id }待处理{ $delay }秒 (已等待{ $elapsed }s)
...
msg-32fbf7c1 = 已添加{ $res }组件到媒体群组{ $media_group_id }
msg-23bae28a = Telegram 适配器已关闭。
msg-e46e7740 = 关闭Telegram适配器时发生错误：{ $e }

### astrbot/core/platform/sources/slack/client.py

msg-1d6b68b9 = Slack请求签名验证失败
msg-53ef18c3 = 已收到Slack事件：{ $event_data }
msg-58488af6 = 处理 Slack 事件时出错:{ $e }
msg-477be979 = Slack Webhook 服务器启动中，监听{ $res }待翻译文本：{ $res_2 }{ $res_3 }待翻译文本：
msg-639fee6c = Slack Webhook 服务器已停止
msg-a238d798 = 套接字客户端未初始化
...
msg-b67bcee0 = WebSocket connection is not established
...
msg-7db44899 = Satori 连接成功 - 机器人{ $res }: platform={ $platform }用户ID={ $user_id }用户名={ $user_name }
...
msg-ca6dca7f = Failed to convert referenced message:{ $e }
...
msg-10237240 = 处理来自对话的消息时出错{ $conversation_id }待翻译文本：{ $e }
...
msg-c165744d = QQ官方机器人接口适配器已被优雅地关闭
...
msg-040e7942 = [QQOfficial] Markdown 发送被拒绝，已回退到内容模式重试。
...
msg-e6f0c96f = 提供者类型{ $provider_type_name }未注册
msg-c7953e3f = 批次{ $batch_idx }处理失败，已重试{ $max_retries }次:{ $e }
msg-10f72727 = { $error_msg }
msg-7ff71721 = 重排服务提供者测试失败，未返回任何结果
...
msg-93c54ce0 = 测试连接后正在清理MCP客户端。
msg-368450ee = 此函数调用工具所属的插件{ $res }此功能已被禁用，请在管理面板中启用后再激活此工具。
msg-4ffa2135 = Loading MCP configuration failed:{ $e }
...
msg-7fc6f623 = 图片{ $image_url }The result is empty and will be ignored.

### astrbot/core/provider/manager.py

msg-9e1a7f1f = Provider{ $provider_id }不存在，无法设置。
msg-5fda2049 = 未知的提供者类型：{ $provider_type }
msg-a5cb19c6 = 没有找到 ID 为{ $provider_id }的提供商，这可能是由于您修改了提供商（模型）ID 导致的。
msg-78b9c276 = { $res }
msg-5bdf8f5c = { $e }
msg-b734a1f4 = 提供程序{ $provider_id }配置项 key{ $idx }使用环境变量{ $env_key }但未设置。
...
msg-4636f83c = No applicable found{ $res }待翻译文本：{ $res_2 }) 的提供商适配器，请检查是否已经安装或者名称填写错误。已跳过。
msg-e9c6c4a2 = 无法找到{ $res }的类
msg-f705cf50 = Provider 类{ $cls_type }不是STTProvider的子类
msg-d20620aa = 已选择{ $res }待翻译文本：{ $res_2 }) 作为当前语音转文本提供商适配器。
...
msg-186fd5c6 = 实例化{ $res }待翻译文本：{ $res_2 }) 提供商适配器失败：{ $e }
msg-ede02a99 = 用户配置中的提供者：{ $config_ids }
msg-95dc4227 = 自动选择{ $res }作为当前提供商适配器。
msg-a6187bac = 自动选择{ $res }作为当前语音转文本提供商适配器。
msg-bf28f7e2 = 自动选择{ $res }作为当前文本转语音提供商适配器。
msg-dba10c27 = 终止{ $provider_id }Provider Adapter{ $res },{ $res_2 },{ $res_3 }) ...
msg-9d9d9765 = { $provider_id }Provider adapter has been terminated({ $res },{ $res_2 },{ $res_3 })
msg-925bb70a = 提供者{ $target_prov_ids }已从配置中删除。
msg-a1657092 = 新提供程序配置必须包含一个'id'字段。
...
msg-d3312319 = Result{ $idx }缺少 relevance_score，使用默认值 0.0
...
msg-aa7c77a5 = 无效的思考级别：{ $thinking_level }使用 HIGH
...
msg-73541852 = Model-generated content failed Gemini platform security checks
...
msg-cd690916 = 温度参数已超过最大值2，仍然发生重复读取
msg-632e23d7 = 发生了重算，正在提高温度至{ $temperature }重试...
msg-41ff84bc = { $model }不支持系统提示，已自动移除（影响角色设定）
...
msg-6c2c0426 = Anthropic不支持的图片URL格式：{ $res }...
...
msg-6e47d22a = API 调用失败，重试{ $max_retries }It still fails.
msg-974e7484 = 未知错误
msg-7fc6f623 = Image{ $image_url }得到的结果为空，将忽略。
msg-0b041916 = 不支持的额外内容块类型:{ $res }
...
msg-012edfe1 = Gemini TTS API 未返回任何音频内容。
...
msg-3303e3a8 = Genie TTS 未能为以下内容生成音频：{ $text }
msg-1cfe1af1 = Genie TTS 流错误：{ $e }

### astrbot/core/provider/sources/dashscope_tts.py

msg-f23d2372 = Dashscope TTS 模型未配置。
msg-74a7cc0a = 音频合成失败，返回空内容。可能是模型不受支持或服务不可用。
msg-bc8619d3 = dashscope SDK缺少MultiModalConversation模块。请升级dashscope包以使用Qwen TTS模型。
msg-95bbf71e = 未为Qwen TTS模型指定语音，将使用默认语音'Cherry'。
msg-3c35d2d0 = 模型 '{$modelName}' 的音频合成失败{ $model }'.'{ $response }
...
msg-7bcb6e1b = Xinference Rerank: 未提供API密钥。
msg-b0d1e564 = 模型{ $res }' 已经在以 UID: 运行{ $uid }
msg-16965859 = 启动中{ $res }模型...
msg-7b1dfdd3 = 模型已启动。
msg-3fc7310e = 模型{ $res }未在运行，且自动启动功能已禁用。提供商将不可用。
msg-15f19a42 = 初始化Xinference模型失败：{ $e }
msg-01af1651 = Xinference初始化失败，异常信息：{ $e }
msg-2607cc7a = Xinference重排序模型尚未初始化。
msg-3d28173b = 重排API响应：{ $response }
msg-4c63e1bd = Rerank API 返回了空列表。原始响应：{ $response }
msg-cac71506 = Xinference 重新排序失败：{ $e }
msg-4135cf72 = Xinference 重排失败，异常信息：{ $e }
msg-ea2b36d0 = 正在关闭Xinference重排客户端...
...
msg-5c7dee08 = 订阅密钥格式无效，应为32位字母数字或其他[...]格式
...
msg-b0d1e564 = 模型{ $res }正在以UID运行的进程：{ $uid }
...
msg-bbc43272 = 音频下载失败{ $audio_url }, 状态:{ $res }
msg-f4e53d3d = 文件未找到：{ $audio_url }
msg-ebab7cac = 音频字节为空。
msg-7fd63838 = 音频需要转换（{ $conversion_type }正在使用临时文件...
msg-d03c4ede = 正在将丝绸转换为WAV格式...
msg-79486689 = 正在将amr转换为wav ...
msg-c4305a5b = Xinference STT 结果：{ $text }
msg-d4241bd5 = Xinference STT 转写失败，状态为{ $res }待翻译文本：{ $error_text }
msg-8efe4ef1 = Xinference STT 失败：{ $e }
msg-b1554c7c = Xinference STT 出现异常：{ $e }
msg-9d33941a = 已删除临时文件：{ $temp_file }
msg-7dc5bc44 = 未能移除临时文件{ $temp_file }待翻译文本：{ $e }
...
msg-4d57bc4f = [GSV TTS] 提供者HTTP会话未就绪或已关闭。
msg-2a4a0819 = [GSV TTS] 请求地址：{ $endpoint }参数：{ $params }
msg-5fdee1da = [GSV TTS] 请求发送至{ $endpoint }执行失败，状态异常{ $res }待翻译文本：{ $error_text }
...
msg-1921aa47 = 不是一个有效的文件：{ $url }
msg-2ee3827c = 生成的视频文件回调链接：{ $payload_file }
msg-32f4fc78 = 未提供有效的文件或 URL
msg-36375f4c = Do not synchronously await downloads in asynchronous contexts! This warning usually occurs when certain logic attempts to retrieve the file content of a file message segment via <File>.file. Please use await get_file() instead of directly accessing the <File>.file field.
...
msg-314258f2 = Failed to save metric to database:{ $e }
...
msg-215a0cfc = [媒体工具] ffmpeg音频转换失败：{ $error_msg }
msg-8cce258e = ffmpeg 转换失败：{ $error_msg }
msg-f0cfcb92 = [媒体工具] 音频转换成功：{ $audio_path }->{ $output_path }
msg-ead1395b = [媒体工具] ffmpeg未安装或不在PATH环境变量中，无法转换音频格式。请安装ffmpeg: https://ffmpeg.org/
msg-5df3a5ee = 未找到 ffmpeg
msg-6322d4d2 = [Media Utils] 转换音频格式时出错:{ $e }
msg-e125b1a5 = [媒体工具] 已清理失败的{ $output_format }Output file:{ $output_path }
msg-5cf417e3 = [媒体工具] 清理失败的{ $output_format }输出文件时出错:{ $e }
msg-3766cbb8 = [媒体工具] ffmpeg视频转换失败：{ $error_msg }
msg-77f68449 = [Media Utils] 视频转换成功:{ $video_path }->{ $output_path }
msg-3fb20b91 = [媒体工具] ffmpeg未安装或不在PATH环境变量中，无法转换视频格式。请安装ffmpeg: https://ffmpeg.org/
msg-696c4a46 = [媒体工具] 转换视频格式时出错:{ $e }
msg-98cc8fb8 = [媒体工具] 清理失败的音频输出文件时出错:{ $e }
msg-3c27d5e8 = [媒体工具] 清理失败的视频封面文件时出错:{ $e }
...
msg-fb7718cb = 解析对话历史失败：%s

### astrbot/core/utils/io.py

msg-665b0191 = SSL证书验证失败{ $url }正在禁用SSL验证（CERT_NONE）作为备用方案。这种做法不安全，会使应用程序暴露于中间人攻击的风险中。请检查并解决证书问题。
msg-04ab2fae = 下载文件失败:{ $res }
msg-63dacf99 = 文件大小:{ $res }KB | 文件地址:{ $url }
msg-14c3d0bb = 下载进度:{ $res }速度:{ $speed }千字节/秒
msg-4e4ee68e = SSL 证书验证失败，已关闭 SSL 验证（不安全，仅用于临时下载）。请检查目标服务器的证书配置。
msg-5a3beefb = SSL证书验证失败{ $url }正在回退到未验证的连接（CERT_NONE）。此操作不安全，会使应用程序面临中间人攻击的风险。请检查远程服务器的证书问题。
...
msg-78b9c276 = { $res }
msg-e21f1509 = 迁移供应商{ $res }到新结构
msg-dd3339e6 = 提供商-源结构迁移完成
msg-1cb6c174 = 从版本 4.5 迁移至 4.6 失败：{ $e }
msg-a899acc6 = Webchat 会话迁移失败：{ $e }
msg-b9c52817 = token_usage 列的迁移失败：{ $e }
msg-d9660ff5 = 迁移提供者源结构失败：{ $e }
...
msg-391449f0 = 临时目录超出限制 ({ $total_size }待翻译文本：{ $limit }已移除{ $removed_files }文件，已发布{ $released }字节数（目标{ $target_release }字节数。
msg-aaf1e12a = 临时目录清理器已启动。interval={ $res }清理比例={ $res_2 }
...
msg-aa9e40b8 = pip模块不可用 (sys.executable={ $res }, frozen={ $res_2 }ASTRBOT_DESKTOP_CLIENT={ $res_3 })
msg-560f11f2 = 读取依赖文件失败，跳过冲突检测: %s
msg-91ae1d17 = 读取 site-packages 元数据失败，使用回退模块名: %s
msg-c815b9dc = { $conflict_message }
msg-e8d4b617 = 已从插件 site-packages 加载 %s：%s
msg-4ef5d900 = 从插件站点包中恢复依赖项 %s，同时首选 %s。
msg-0bf22754 = 插件site-packages中未找到模块%s：%s
msg-76a41595 = 无法从插件站点包中首选模块 %s：%s
msg-3d4de966 = 无法为加载器 %s (%s) 修补 pip distlib 查找器：%s
msg-117d9cf4 = Distlib查找器补丁未对加载器%s (%s)生效。
msg-b7975236 = 已修补用于冻结加载器的 pip distlib 查找器：%s (%s)
msg-b1fa741c = 跳过修补分发库查找器，因为_finder_registry不可用。
...
msg-ea7c80f1 = 原文：{ $provider_label }网络连接失败{ $error_type })。错误:{ $error }
...
msg-24049c48 = quoted_message_parser：在%d次跳转后停止获取嵌套转发消息

### astrbot/core/utils/quoted_message/onebot_client.py

msg-062923e6 = quoted_message_parser: 操作 %s 失败，参数 %s: %s
msg-f33f59d5 = quoted_message_parser: 操作 %s 的所有尝试均失败，last_params=%s，错误=%s

### astrbot/core/utils/quoted_message/image_resolver.py

msg-94224a01 = quoted_message_parser: 跳过非图像本地路径 ref=%s
msg-3e6c0d14 = quoted_message_parser: 在 %d 次操作后未能解析引用的图片 ref=%s

### astrbot/core/agent/tool_image_cache.py

msg-45da4af7 = ToolImageCache 已初始化，缓存目录：{ $res }
msg-017bde96 = 已保存工具镜像至：{ $file_path }
msg-29398f55 = 保存工具镜像失败：{ $e }
msg-128aa08a = 读取缓存图片失败{ $file_path }待翻译文本：{ $e }
msg-3c111d1f = 缓存清理过程中出错：{ $e }
msg-eeb1b849 = 已清理{ $cleaned }已过期的缓存图像

### astrbot/core/agent/message.py

msg-d38656d7 = { $invalid_subclass_error_msg }
msg-42d5a315 = 无法验证{ $value }作为内容部件
msg-ffc376d0 = 除非角色为'assistant'且tool_calls不为None，否则内容为必填项。

### astrbot/core/agent/mcp_client.py

msg-6a61ca88 = 警告：缺少 'mcp' 依赖项，MCP 服务将不可用。
msg-45995cdb = 警告：缺少 'mcp' 依赖项或MCP库版本过旧，可流式传输的HTTP连接不可用。
msg-2866b896 = MCP 连接配置缺少传输或类型字段
...
msg-a0d672dc = 压缩已触发，开始压缩...
msg-e6ef66f0 = 压缩完成。{ $prev_tokens }->{ $tokens_after_summary }令牌，压缩率：{ $compress_rate }%.
msg-3fe644eb = 上下文在压缩后仍然超过最大令牌数，正在应用折半截断处理...

### astrbot/core/agent/runners/tool_loop_agent_runner.py

msg-960ef181 = 已从 %s 切换到备用聊天服务提供商：%s
msg-4f999913 = 聊天模型 %s 返回错误响应，尝试降级到下一个提供程序。
msg-c042095f = 聊天模型 %s 请求错误: %s
...
msg-ed80313d = LLM返回了空的助手消息，且未调用任何工具。
msg-970947ae = 已添加{ $res }已缓存的图像已发送至上下文以供LLM审核
msg-6b326889 = 代理已达到最大步数限制 ({ $max_step })，强制进行最终响应。
...
msg-34c13e02 = on_tool_end 钩子中出现错误：{ $e }
msg-78b9c276 = { $res }
...
msg-55333301 = 请求未设置。请先调用 reset()。
msg-d3b77736 = on_agent_begin 钩子中出现错误：{ $e }
msg-e3af4efd = 阿里云百炼请求失败：{ $res }
msg-fccf5004 = dashscope 流式数据块：{ $chunk }
msg-100d7d7e = 阿里云百炼请求失败: request_id={ $res }待翻译文本：, 代码={ $res_2 }，消息={ $res_3 }, 请参考文档：https://help.aliyun.com/zh/model-studio/developer-reference/error-code
msg-10f72727 = { $error_msg }
...
msg-2d9e1c08 = Failed to process image{ $url }待翻译文本：{ $e }
msg-1f50979d = { $content }
msg-6fe5588b = Coze消息已处理完成
msg-d2802f3b = Coze 聊天完成
...
msg-696dad99 = Failed to process image{ $image_url }:{ $e }
...
msg-3972f693 = dify 响应分块：{ $chunk }
msg-6c74267b = Dify 消息结束
msg-1ce260ba = Dify 出现错误：{ $chunk }
msg-a12417dd = Dify 出现错误 status:{ $res }消息:{ $res_2 }
...
msg-16cc2a7a = 插件{ $res }In Session{ $session_id }Disabled in, skipping processor{ $res_2 }

### astrbot/core/star/star_manager.py

msg-bfa28c02 = watchfiles is not installed, hot reload of plugins cannot be implemented.
msg-f8e1c445 = 插件热重载监视任务异常:{ $e }
msg-78b9c276 = { $res }
msg-28aeca68 = Detected file changes:{ $changes }
msg-aeec7738 = 检测到插件{ $plugin_name }文件变化，正在重载...
msg-4f989555 = 插件{ $d }未找到 main.py 或者{ $d }.py，跳过。
msg-74b32804 = 正在安装插件{ $p }所需的依赖库:{ $pth }
msg-936edfca = Update plugin{ $p }Dependency failed. Code:{ $e }
msg-ebd47311 = 插件{ $root_dir_name }Import failed, attempting to recover from installed dependencies:{ $import_exc }
...
msg-e0002829 = 插件{ $res }Not properly terminated:{ $e }, 可能会导致该插件运行不正常。
...
msg-64edd12c = 钩子函数(on_plugin_loaded){ $res }-{ $res_2 }
msg-db49f7a1 = ----- 插件{ $root_dir_name }载入失败
msg-26039659 = 待翻译文本：{ $line }
msg-4292f44d = 待翻译文本：----------------------------------
msg-d2048afe = 同步指令配置失败:{ $e }
msg-df515dec = 已清理安装失败的插件目录：{ $plugin_path }
msg-1f2aa1a9 = Failed to clean up the installation directory for the failed plugin:{ $plugin_path }，原因:{ $e }
...
msg-e247422b = 插件{ $plugin_name }Not properly terminated{ $e }, 可能导致资源泄漏等问题。
msg-0c25dbf4 = 插件{ $plugin_name }数据不完整，无法卸载。
msg-d6f8142c = 插件已成功移除，但删除插件文件夹失败：{ $e }您可以手动删除该文件夹，位于 addons/plugins/ 下。
msg-6313500c = 已删除插件{ $plugin_name }的配置文件
msg-f0f01b67 = 删除插件配置文件失败:{ $e }
msg-c4008b30 = 已删除插件{ $plugin_name }持久化数据 (plugin_data)
...
msg-e1853811 = 移除了插件{ $plugin_name }的处理函数{ $res }待翻译文本：({ $res_2 })
msg-95b20050 = Removed plugin{ $plugin_name }的平台适配器{ $adapter_name }
msg-9f248e88 = 该插件是 AstrBot 保留插件，无法更新。
msg-ff435883 = 正在终止插件{ $res }...
msg-355187b7 = 插件{ $res }Not activated, no termination required, skipping.
...
msg-7b90d0e9 = 会话{ $session_id }TTS状态已更新为：{ $res }
...
msg-e3b5cdfb = Configuration item{ $key }不存在。
...
msg-e77ccce6 = Unable to retrieve module{ $res }Metadata information of
msg-76ac38ee = Unable to get plugin name
msg-751bfd23 = Unable to create directory{ $data_dir }权限不足
msg-68979283 = 无法创建目录{ $data_dir }待翻译文本：{ $e }

### astrbot/core/star/context.py

msg-60eb9e43 = 提供方{ $chat_provider_id }未找到
msg-da70a6fb = 代理未生成最终LLM响应
msg-141151fe = 未找到提供商
msg-a5cb19c6 = 未找到 ID 为{ $provider_id }的提供商，这可能是由于您修改了提供商（模型）ID 导致的。
msg-2a44300b = The type of the conversation model (provider) for this session source is incorrect:{ $res }
...
msg-df2c7e1b = 删除旧版本插件{ $plugin_path }Folder failed:{ $e }使用覆盖安装。
...
msg-0dd9b70d = Parse instruction processing function{ $res }失败，跳过该指令。原因:{ $e }
...
msg-64619f8e = 'register_star' 装饰器已被弃用，将在未来版本中移除。

### astrbot/core/star/register/star_handler.py

msg-7ff2d46e = 注册指令{ $command_name }The sub-command parameter was not provided when executing the sub-command.
msg-b68436e1 = 注册裸指令时未提供 command_name 参数。
msg-1c183df2 = { $command_group_name }指令组的子指令组 sub_command 未指定
msg-9210c7e8 = 根指令组的名称未指定
msg-678858e7 = Failed to register the command group.
...
msg-04dbdc3a = Required parameters are missing. The complete parameters for this command are:{ $res }
...
msg-8f3eeb6e = 操作数必须是CustomFilter的子类。
msg-732ada95 = CustomFilter类只能与其他CustomFilter类一起操作。
msg-51c0c77d = CustomFilter 类只能与其他 CustomFilter 类共同操作。
...
msg-385bc50a = 已生成嵌入{ $res }内容在{ $res_2 }秒。
...
msg-7805b529 = 迁移{ $total_cnt }迁移旧的会话数据到新的表中...
msg-6f232b73 = 进度:{ $progress }% ({ $res }待翻译文本：{ $total_cnt })
msg-6b1def31 = 未找到该条旧会话对应的具体数据:{ $conversation }跳过。
msg-b008c93f = 迁移旧会话{ $res }失败:{ $e }
msg-6ac6313b = 成功迁移{ $total_cnt }Migrate old session data to the new table.
msg-6b72e89b = 迁移旧平台数据，offset_sec:{ $offset_sec }秒。
msg-bdc90b84 = 迁移{ $res }正在将旧的平台数据迁移到新的表中...
msg-e6caca5c = 未找到旧平台数据，跳过迁移。
msg-1e824a79 = 进度:{ $progress }% ({ $res }待翻译文本：{ $total_buckets })
msg-813384e2 = 迁移平台统计数据失败:{ $platform_id },{ $platform_type }, 时间戳:{ $bucket_end }
msg-27ab191d = 成功迁移{ $res }迁移旧的平台数据到新表。
msg-8e6280ed = 迁移{ $total_cnt }正在迁移旧的 WebChat 会话数据到新的表中...
msg-cad66fe1 = 迁移旧 WebChat 会话{ $res }失败
msg-63748a46 = 成功迁移{ $total_cnt }Migrate old WebChat session data to the new table.
msg-dfc93fa4 = 迁移{ $total_personas }正在将 Persona 配置迁移到新表...
...
msg-ee03c001 = 迁移会话{ $umo }Failed to get conversation data:{ $e }
msg-5c4339cd = 迁移会话{ $umo }的服务配置到新表成功，平台 ID:{ $platform_id }
msg-4ce2a0b2 = 迁移会话{ $umo }服务配置失败:{ $e }
msg-2e62dab9 = 迁移会话{ $umo }变量失败:{ $e }
msg-afbf819e = 迁移会话{ $umo }Provider preference successfully migrated to new table, platform ID:{ $platform_id }
...
msg-cf287e58 = Session{ $session_id }已存在，跳过
...
msg-7b3dc642 = - 第 {$attempt} 次尝试时 LLM 调用失败{ $res }待翻译文本：{ $res_2 }错误：{ $res_3 }
msg-4ba9530f = - 处理块后失败{ $res }尝试次数。使用原文。
msg-77670a3a = 知识库{ $res }未配置 Embedding Provider
msg-8e9eb3f9 = Cannot find ID{ $res }的嵌入提供程序
msg-3e426806 = Cannot find the ID{ $res }的重新排序提供商
...
msg-969b17ca = Failed to clean up multimedia files{ $media_path }待翻译文本：{ $me }
msg-18d25e55 = 无法找到 ID 为{ $doc_id }Documentation
msg-f5d7c34c = 错误：Tavily API 密钥未在 provider_settings 中配置。
msg-975d88e0 = 无法从URL提取内容{ $url }:{ $e }
msg-cfe431b3 = 未从URL提取到内容：{ $url }
msg-e7f5f836 = Content cleaning did not extract valid text. Please try turning off the content cleaning feature or retry after switching to a higher-performance LLM model.
...
msg-31963d3f = 无法找到 ID 为{ $cleaning_provider_id }LLM Provider 或类型不正确
msg-82728272 = 初步分块完成，生成{ $res }个块用于修复。
msg-6fa5fdca = 块{ $i }处理异常：{ $res }. 回退到原始块。
msg-6780e950 = Text restoration completed:{ $res }个原始块 ->{ $res_2 }个最终块。
...
msg-78b9c276 = { $res }
msg-9349e112 = KnowledgeBase数据库已初始化：{ $DB_PATH }
msg-7605893e = 创建知识库时必须提供embedding_provider_id
msg-0b632cbd = 知识库名称{ $kb_name }已存在
msg-ca30330f = 关闭知识库{ $kb_id }失败:{ $e }
msg-00262e1f = Failed to close knowledge base metadata database:{ $e }
...
msg-7b14cdb7 = Tavily网页提取失败：{ $reason }，状态：{ $res }
msg-cfe431b3 = 未从URL中提取到任何内容：{ $url }
msg-b0897365 = 获取URL失败{ $url }:{ $e }
msg-975d88e0 = 未能从URL提取内容{ $url }待翻译文本：{ $e }
...
msg-c0656f4e = 块重叠必须为非负值
msg-82bd199c = chunk_overlap 必须小于 chunk_size

### astrbot/core/knowledge_base/retrieval/manager.py

msg-fcc0dde2 = 知识库 ID{ $kb_id }Instance not found, skipped retrieval for this knowledge base.
msg-320cfcff = 密集检索跨域{ $res }基地遭受攻击{ $res_2 }s 并返回{ $res_3 }结果。
msg-90ffcfc8 = 稀疏检索跨{ $res }基地占领{ $res_2 }已发送并返回{ $res_3 }结果。
msg-12bcf404 = 排名融合耗时{ $res }和返回{ $res_2 }结果。
...
msg-73f9cf65 = 上传的文件不是有效的zip归档文件。
msg-69eb5f95 = 压缩文件为空。
msg-9e9abb4c = { $top_dirs }
msg-20b8533f = ZIP归档必须包含一个顶级文件夹。
msg-1db1caf7 = 技能文件夹名称无效。
msg-d7814054 = 压缩包包含绝对路径。
msg-179bd10e = ZIP 压缩包包含无效的相对路径。
msg-90f2904e = 压缩归档包含意外的顶级条目。
msg-95775a4d = 技能文件夹中未找到 SKILL.md 文件。
...
msg-c046b6e4 = { $msg }
msg-0e6f1f5d = 开始从{ $zip_path }Import Backup
...
msg-89a2120c = Import Table{ $table_name }待翻译文本：{ $count }条记录
msg-f1dec753 = Import knowledge base records into{ $table_name }失败:{ $e }
msg-9807bcd8 = Failed to import document blocks:{ $e }
msg-98a66293 = Import attachment{ $name }失败:{ $e }
msg-39f2325f = 备份版本不支持目录备份，跳过目录导入
msg-689050b6 = 已备份现有目录{ $target_dir }To{ $backup_path }
...
msg-2821fc92 = Export Table{ $table_name }待翻译文本：{ $res }条记录
msg-52b7c242 = 导出表{ $table_name }失败:{ $e }
msg-56310830 = 导出知识库表{ $table_name }待翻译文本：{ $res }条记录
msg-f4e8f57e = Export Knowledge Base Table{ $table_name }失败:{ $e }
msg-8e4ddd12 = Exporting knowledge base documents failed:{ $e }
...
msg-cb773e24 = 导出目录{ $dir_name }待翻译文本：{ $file_count }个文件,{ $total_size }字节
msg-ae929510 = Export Directory{ $dir_path }失败：{ $e }
msg-93e331d2 = Export attachment failed:{ $e }

### astrbot/core/computer/computer_client.py

msg-7cb974b8 = 正在将技能包上传到沙箱环境...
msg-130cf3e3 = 无法将技能包上传到沙盒。
msg-99188d69 = 无法删除临时技能压缩文件：{ $zip_path }
msg-3f3c81da = 未知启动器类型：{ $booter_type }
msg-e20cc33a = 启动会话沙箱时出错{ $session_id }待翻译文本：{ $e }
...
msg-93476abb = 文件{ $remote_path }从沙箱下载到{ $local_path }
msg-079c5972 = 发送文件消息时出错：{ $e }
msg-ce35bb2c = 下载文件时出错{ $remote_path }待翻译文本：{ $e }

### astrbot/core/computer/booters/local.py

msg-487d0c91 = 路径超出了允许的计算机根目录范围。
msg-e5eb5377 = 已阻止不安全Shell命令。
...
msg-b03115b0 = 沙盒船只已就绪：{ $res }对于会话：{ $session_id }
...
msg-fbdbe32f = 正在为会话启动(Boxlite)：{ $session_id }，这可能需要一些时间...
msg-b1f13f5f = Boxlite启动器已为会话启动：{ $session_id }
msg-e93d0c30 = 正在为飞船关闭Boxlite启动程序：{ $res }
msg-6deea473 = 船舶用Boxlite启动器：{ $res }已停止

### astrbot/core/cron/manager.py

msg-724e64a9 = 跳过调度基本cron任务 %s，因为缺少处理器。
msg-78ef135f = cron 作业 %s 的时区 %s 无效，已回退至系统时区。
msg-e71c28d3 = run_once 作业缺少 run_at 时间戳
msg-dd46e69f = 计划定时任务失败{ $res }待翻译文本：{ $e }
msg-aa2e4688 = 未知的定时任务类型：{ $res }
msg-186627d9 = 定时任务{ $job_id }失败：{ $e }
msg-cb955de0 = 未找到基础cron任务处理程序{ $res }
msg-2029c4b2 = ActiveAgentCronJob 缺少会话。
msg-6babddc9 = 定时任务会话无效：{ $e }
msg-865a2b07 = 构建 cron 作业的主代理失败。
msg-27c9c6b3 = Cron任务代理未收到响应

### astrbot/utils/http_ssl_common.py

msg-7957c9b6 = 无法将certifi CA证书包加载到SSL上下文中；仅回退到系统信任存储：%s
...
msg-f0b6145e = Failed to download management panel:{ $e }
msg-9504d173 = Initializing management panel directory...
...
msg-bffc8bfa = 插件{ $name }Created successfully
msg-08eae1e3 = 未安装任何插件
msg-1a021bf4 = No installable plugins found{ $name }，可能是不存在或已安装
...
msg-cb049eb2 = AstrBot WebUI 已经优雅地关闭
...
msg-c5170c27 = Download management panel file failed:{ $e }
...
msg-7060da1d = 接收分片: upload_id={ $upload_id }, chunk={ $res }待翻译文本：{ $total_chunks }
...
msg-5ea3d72c = 找不到该任务
msg-f0901aef = Failed to fetch task progress:{ $e }
...
msg-8b0a48ee = 提供商{ $res }(ID:{ $res_2 }) 已就绪。
...
msg-92347c35 = Provider{ $provider_id }Type does not support fetching model list
...
msg-8361e44d = Not found{ $provider_type }的类
msg-4325087c = 提供商不是 EmbeddingProvider 类型
msg-a9873ea4 = 检测到{ $res }Embedding vector dimension is{ $dim }
msg-d170e384 = 获取嵌入维度失败:{ $e }
msg-abfeda72 = 缺少参数 source_id
msg-0384f4c9 = ID not found{ $provider_source_id }的 provider_source
msg-aec35bdb = provider_source 缺少 type 字段
msg-cbb9d637 = Failed to dynamically import provider adapter:{ $e }
msg-468f64b3 = Provider{ $provider_type }不支持获取模型列表
...
msg-1199c1f9 = 使用平台缓存的徽标令牌{ $res }
msg-889a7de5 = 平台类未找到{ $res }
msg-317f359c = 平台注册的Logo代币{ $res }
msg-323ec1e2 = 平台{ $res }logo文件未找到：{ $logo_file_path }
msg-bc6d0bcf = 导入平台所需的模块失败{ $res }待翻译文本：{ $e }
msg-b02b538d = 平台文件系统错误{ $res }logo:{ $e }
msg-31123607 = 注册平台徽标时出现意外错误{ $res }待翻译文本：{ $e }
msg-af06ccab = 配置文件{ $conf_id }不存在
...
msg-787a5dea = Failed to fetch knowledge base statistics:{ $e }
...
msg-a168d76d = [实时聊天] 时间戳不匹配或未处于发言状态：{ $stamp }vs{ $res }
...
msg-b35a375c = [实时聊天] 用户打断:{ $res }
msg-2c3e7bbc = [Live Chat] STT 提供商未配置
msg-0582c8ba = [Live Chat] STT 识别结果为空
msg-57c2b539 = [Live Chat] STT 结果:{ $user_text }
msg-6b7628c6 = [在线聊天] 检测到用户打断
...
msg-140caa36 = [在线聊天] 保存中断消息:{ $interrupted_text }
msg-869f51ea = [在线聊天] 用户消息：{ $user_text }(session:{ $res }, ts:{ $timestamp })
...
msg-98aa3644 = 导出对话失败: user_id={ $user_id }, 会话ID={ $cid }，错误={ $e }
...
msg-8bc87eb5 = 无效载荷
msg-29f616c2 = 需要会话
msg-ae7c99a4 = 当 run_once=true 时，run_at 是必需的。
msg-4bb8c206 = cron_expression 在 run_once=false 时必须提供
msg-13fbf01e = run_at 必须为 ISO 日期时间格式
...
msg-78b9c276 = { $res }
msg-977490be = Failed to fetch MCP server list:{ $e }
...
msg-04827ead = 缺少键值：title
msg-34fccfbb = 缺少密钥：project_id
...
msg-979c6e2f = 删除 umo{ $umo }The rule failed:{ $e }
msg-77d2761d = 批量删除会话规则失败:{ $e }
msg-6619322c = Failed to get UMO list:{ $e }
...
msg-51d84afc = Delete personality failed:{ $e }{ "\u000A" }{ $res }
...
msg-22020727 = Failed to delete the folder:{ $e }
msg-7a69fe08 = items 不能为空
msg-e71ba5c2 = 每个项目必须包含 id、type 和 sort_order 字段
msg-dfeb8320 = type 字段必须是 'persona' 或 'folder'
msg-aec43ed3 = 更新排序失败:{ $e }{ "\u000A" }{ $res }
msg-75ec4427 = Update ordering failed:{ $e }
...
msg-1b79360d = 无效的作用域
msg-d6621696 = expires_in_days必须为整数
...
msg-c6ec40ff = 消息内容为空（仅回复不被允许）
msg-2c3fdeb9 = 消息均为空
msg-9bc95e22 = session_id为空
...
msg-80bbd0ff = WebChat 流式传输意外错误：{ $e }
msg-dbf41bfc = 缺少键值: session_id
msg-d922dfa3 = 会话{ $session_id }未找到
msg-c52a1454 = 权限被拒绝
msg-9d7a8094 = 在会话清理期间删除UMO路由 %s 失败：%s
...
msg-6ac25100 = Request{ $url }失败，错误：{ $e }
...
msg-dedcd957 = Unable to retrieve remote MD5, using cache.
...
msg-6d1235b6 = Uninstall plugin{ $plugin_name }成功
...
msg-cb230226 = 停用插件{ $plugin_name }待翻译文本：
msg-abc710cd = /api/plugin/off:{ $res }
msg-06e2a068 = Enable Plugin{ $plugin_name }待翻译文本：
msg-82c412e7 = /api/plugin/on:{ $res }
msg-77e5d67e = 正在获取插件{ $plugin_name }README文件内容
msg-baed1b72 = 插件名称为空
msg-773cca0a = Plugin name cannot be empty
msg-082a5585 = 插件{ $plugin_name }不存在
msg-ba106e58 = 插件{ $plugin_name }Directory does not exist
msg-e38e4370 = Cannot find plugin directory:{ $plugin_dir }
msg-df027f16 = Plugin not found{ $plugin_name }的目录
msg-5f304f4b = 插件{ $plugin_name }没有README文件
msg-a3ed8739 = /api/plugin/readme:{ $res }
msg-2f9e2c11 = Failed to read README file:{ $e }
msg-dcbd593f = 正在获取插件{ $plugin_name }更新日志
msg-ea5482da = /api/plugin/changelog:{ $res }
msg-8e27362e = 读取更新日志失败:{ $e }
msg-0842bf8b = 插件{ $plugin_name }No changelog file available
...
msg-8b56b437 = Session{ $uid }变量{ $key }存储成功。使用 /unset 移除。
...
msg-b435fcdc = 提供程序可达性检查失败：id=%s type=%s code=%s reason=%s
...
msg-4d4f587f = Switch model to{ $res }待翻译文本：
...
msg-a90c75d4 = Cancel Personality Successful.
...
msg-bed9def5 = web_searcher - 网页抓取中：{ $res }-{ $res_2 }
msg-8214760c = 必应搜索错误：{ $e }，试试下一个...
msg-8676b5aa = 搜索必应失败
msg-3fb6d6ad = sogo 搜索错误：{ $e }
msg-fe9b336f = 搜索搜狗失败
msg-c991b022 = 错误：AstrBot中未配置Tavily API密钥。
...
msg-b36134c9 = 错误：Tavily API 密钥未在 AstrBot 中配置。
msg-98ed69f4 = 错误：网址必须是非空字符串。
msg-51edd9ee = 错误：BoCha API密钥未在AstrBot中配置。
msg-73964067 = BoCha 网络搜索失败：{ $reason }, 状态:{ $res }
...
msg-e3b4d1e9 = Bing搜索失败

### astrbot/builtin_stars/astrbot/main.py

msg-3df554a1 = 聊天增强错误：{ $e }
msg-5bdf8f5c = { $e }
msg-bb6ff036 = No LLM provider found. Please configure it first. Unable to reply actively.
msg-afa050be = 当前未处于对话状态，无法主动回复，请确保平台设置->会话隔离(unique_session)未开启，并使用 /switch 序号 切换或者 /new 创建一个会话。
...
msg-5bdf8f5c = { $e }
msg-8e11fa57 = 没有找到 ID 为{ $image_caption_provider_id }Provider of
msg-8ebaa397 = Provider type error{ $res })，无法获取图片描述
msg-30954f77 = 图片 URL 为空
msg-62de0c3e = 获取图片描述失败:{ $e }
msg-d0647999 = 负载均衡器{ $res }|{ $final_message }
...
msg-dcfbbe82 = 未找到消息{ $ftl_path }
msg-ccd5a28f = Total{ $res }条文本，使用{ $max_workers }Processing concurrent threads for translation...
msg-00b24d69 = { "\u000A" }[Error] 翻译失败，保留原文:{ $e }
msg-ebcdd595 = { "\u000A" }翻译完成，已保存到{ $ftl_path }
