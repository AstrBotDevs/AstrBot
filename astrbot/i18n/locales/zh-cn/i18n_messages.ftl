...
msg-4867eefb = SubAgent persona {$persona_id} not found, fallback to inline prompt.
...
msg-24bd9273 = Error occurred while retrieving knowledge base: {$exc}
msg-36dc1409 = Moonshot AI API key for file extract is not set
msg-b41a7a58 = Unsupported file extract provider: {$res}
...
msg-6097bd34 = Processing image caption with provider: {$provider_id}
msg-7f5e3367 = 处理图片描述失败: {$exc}
msg-719d5e4d = No provider found for image captioning in quote.
msg-633f992f = 处理引用图片失败: {$exc}
msg-1891edf8 = Group name display enabled but group object is None. Group ID: {$res}
msg-7d93dc13 = 时区设置错误: {$exc}, 使用本地时区
msg-09eb6259 = Provider {$provider} does not support image, using placeholder.
msg-f57d475e = Provider {$provider} does not support tool_use, clearing tools.
msg-2e3df24a = sanitize_context_by_modalities applied: removed_image_blocks={$removed_image_blocks}, removed_tool_messages={$removed_tool_messages}, removed_tool_calls={$removed_tool_calls}
msg-5becd564 = Generated chatui title for session {$chatui_session_id}: {$title}
msg-d8cff4db = Unsupported llm_safety_mode strategy: {$res}.
...
msg-c48173dd = Fallback chat provider `{$fallback_id}` not found, skip.
msg-88fd7233 = Fallback chat provider `{$fallback_id}` is invalid type: {$res}, skip.
msg-ee979399 = 未找到任何对话模型（提供商），跳过 LLM 请求处理。
msg-d003c63c = Skip quoted fallback images due to limit={$res} for umo={$res_2}
msg-65bb0f30 = Truncate quoted fallback images for umo={$res}, reply_id={$res_2} from {$res_3} to {$remaining_limit}
msg-617040f3 = Failed to resolve fallback quoted images for umo={$res}, reply_id={$res_2}: {$exc}
msg-d4c7199d = Error occurred while applying file extract: {$exc}
...
msg-12767505 = Captured follow-up message for active agent run, umo={$res}, order_seq={$order_seq}
...
msg-73bf9e45 = Unsupported tool_schema_mode: {$res}, fallback to skills_like
msg-9cdb2b6e = skip llm request: empty message and no provider_request
msg-e461e5af = ready to request llm provider
msg-4d2645f7 = Follow-up ticket already consumed, stopping processing. umo={$res}, seq={$res_2}
msg-abd5ccbc = acquired session lock for llm request
msg-abc0d82d = Provider API base {$api_base} is blocked due to security reasons. Please use another ai provider.
...
msg-6721010c = [QQOfficialWebhook] No cached msg_id for session: {$res}, skip send_by_session
msg-296dfcad = [QQOfficialWebhook] Unsupported message type for send_by_session: {$res}
...
msg-dc6656f8 = [LINE] {$op_name} message failed: status={$res} body={$body}
msg-10996a43 = [LINE] {$op_name} message request failed: {$e}
msg-5aa92977 = [LINE] get content retry failed: message_id={$message_id} status={$res} body={$body}
msg-cf700d79 = [LINE] get content failed: message_id={$message_id} status={$res} body={$body}

### astrbot/core/platform/sources/line/line_event.py
msg-a491ddd0 = [LINE] resolve image url failed: {$e}
msg-ca47546c = [LINE] resolve record url failed: {$e}
msg-616e5840 = [LINE] resolve record duration failed: {$e}
msg-c953a061 = [LINE] resolve video url failed: {$e}
msg-19078257 = [LINE] resolve video cover failed: {$e}
msg-eccdecff = [LINE] generate video preview failed: {$e}
msg-b833dc32 = [LINE] resolve file url failed: {$e}
msg-60290793 = [LINE] resolve file size failed: {$e}
...
msg-321afd59 = [LINE] invalid webhook body: {$e}
...
msg-842a7c69 = Loaded {$module_name} from plugin site-packages: {$module_location}
msg-d93a8842 = Recovered dependency {$dependency_name} while preferring {$module_name} from plugin site-packages.
msg-7632a3cc = Module {$module_name} not found in plugin site-packages: {$site_packages_path}
msg-cd739739 = Failed to prefer module {$module_name} from plugin site-packages: {$reason}
msg-de510412 = Failed to patch pip distlib finder for loader {$res} ({$package_name}): {$exc}
msg-d2de9221 = Distlib finder patch did not take effect for loader {$res} ({$package_name}).
msg-58ebda51 = Patched pip distlib finder for frozen loader: {$res} ({$package_name})
...
msg-80530653 = quoted_message_parser: stop fetching nested forward messages after {$max_fetch} hops

### astrbot/core/utils/quoted_message/onebot_client.py
msg-03ad9f29 = quoted_message_parser: action {$action} failed with params {$res}: {$exc}
msg-519d0dec = quoted_message_parser: all attempts failed for action {$action}, last_params={$res}, error={$last_error}

### astrbot/core/utils/quoted_message/image_resolver.py
msg-6dfa6994 = quoted_message_parser: skip non-image local path ref={$res}
msg-9326ec62 = quoted_message_parser: failed to resolve quoted image ref={$res} after {$res_2} actions
...
msg-ec018aef = Switched from {$res} to fallback chat provider: {$candidate_id}
msg-24b29511 = Chat Model {$candidate_id} returns error response, trying fallback to next provider.
...
msg-2a752c91 = Skip scheduling basic cron job {$res} due to missing handler.
msg-d5c33112 = Invalid timezone {$res} for cron job {$res_2}, fallback to system.
...
msg-5304f0e3 = Failed to load certifi CA bundle into SSL context; falling back to system trust store only: {$exc}
...
msg-d4765667 = Failed to update chat config route for {$umo} with {$config_id}: {$e}
...
msg-7717d729 = Provider reachability check failed: id={$res} type={$res_2} code={$err_code} reason={$err_reason}
