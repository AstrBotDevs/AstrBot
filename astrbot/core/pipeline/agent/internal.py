...
class InternalAgentExecutor:
    ...
    def _select_provider(self, event: AstrMessageEvent):
        ...
        if chain_config and chain_config.chat_provider_id:
            ...
            if not provider:
                logger.error(
                    f"未找到 Chain 配置的提供商: {chain_config.chat_provider_id}。"
                )
            return provider
        try:
    ...
    async def _apply_kb(
        ...
        有两种模式：
        1. 非Agentic模式：由独立的 KnowledgeBaseNode 节点负责检索，
           将结果存储在 event.extra["kb_context"] 中，此处读取并注入
        2. Agentic模式：添加知识库查询工具，让LLM主动调用
        """
        if not self.kb_agentic_mode:
            # 非Agentic模式：从 KnowledgeBaseNode 注入的上下文中读取
    ...
    async def process(
        ...
        try:
            ...
            async with session_lock_manager.acquire_lock(event.unified_msg_origin):
                ...
                # 文件提取功能已由独立的 FileExtractNode 节点处理
                ...
                if action_type == "live":
                    ...
                    yield
                    # yield 后不再检查 done 或保存历史，这些在 stream 消费完后由 wrapped_stream 处理
                    return  # 提前返回，避免后续的 save_to_history
                else:
