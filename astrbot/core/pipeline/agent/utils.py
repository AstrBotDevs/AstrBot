...
class KnowledgeBaseQueryTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(
        ...
        result = await retrieve_knowledge_base(
            ...
            chain_kb_config=(
                context.context.event.chain_config.kb_config
...
async def retrieve_knowledge_base(
    ...
    umo: str,
    context: Context,
    chain_kb_config: dict | None = None,
) -> str | None:
    ...
    if chain_kb_config and "kb_ids" in chain_kb_config:
        ...
        if not kb_ids:
            logger.info("[知识库] Chain 已被配置为不使用知识库")
            return
        ...
        if invalid_kb_ids:
            logger.warning(
                f"[知识库] Chain 配置的以下知识库无效: {invalid_kb_ids}",
            )

        if not kb_names:
            return

        logger.debug(f"[知识库] 使用 Chain 配置，知识库数量: {len(kb_names)}")
    else:
    ...
    if formatted:
        ...
        logger.debug(f"[知识库] 检索到 {len(results)} 条相关知识块")
        return formatted
