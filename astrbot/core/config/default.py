...
CONFIG_METADATA_2 = {
    ...
    "misc_config_group": {
        "metadata": {
            ...
            "dashboard.ssl.auto_acme.enable": {
                "type": "bool",
                "condition": {"dashboard.ssl.enable": True},
            },
            "dashboard.ssl.auto_acme.ip": {
                "type": "string",
                "condition": {"dashboard.ssl.auto_acme.enable": True},
            },
            "dashboard.ssl.auto_acme.email": {
                "type": "string",
                "condition": {"dashboard.ssl.auto_acme.enable": True},
            },
            "dashboard.ssl.auto_acme.server": {
                "type": "string",
                "options": ["letsencrypt", "zerossl"],
                "condition": {"dashboard.ssl.auto_acme.enable": True},
            },
            "dashboard.ssl.auto_acme.keylength": {
                "type": "string",
                "options": ["2048", "ec-256", "ec-384"],
                "condition": {"dashboard.ssl.auto_acme.enable": True},
            },
            "dashboard.ssl.auto_acme.force_issue": {
                "type": "bool",
                "condition": {"dashboard.ssl.auto_acme.enable": True},
            },
        ...
        "metadata": {
            "system": {
                ...
                "items": {
                    ...
                    "dashboard.ssl.auto_acme.enable": {
                        "description": "自动申请 Let's Encrypt IP 证书",
                        "type": "bool",
                        "hint": "启用后，WebUI HTTPS 启动时会使用 acme.sh standalone HTTP-01 模式为当前公网 IP 申请 Let's Encrypt 证书。要求 80 端口可被公网访问且未被占用。",
                        "condition": {"dashboard.ssl.enable": True},
                    },
                    "dashboard.ssl.auto_acme.ip": {
                        "description": "公网 IP",
                        "type": "string",
                        "hint": "留空时自动检测公网 IP。Let's Encrypt 的 IP 证书支持情况可能受 ACME 服务策略影响。",
                        "condition": {"dashboard.ssl.auto_acme.enable": True},
                    },
                    "dashboard.ssl.auto_acme.email": {
                        "description": "ACME 邮箱",
                        "type": "string",
                        "hint": "用于注册 acme.sh 账户和接收证书通知，可留空。",
                        "condition": {"dashboard.ssl.auto_acme.enable": True},
                    },
                    "dashboard.ssl.auto_acme.server": {
                        "description": "ACME CA",
                    ...
                    "dashboard.ssl.auto_acme.force_issue": {
                        "description": "强制重新签发证书",
                        "type": "bool",
                        "hint": "一般保持关闭。开启后每次启动会带 --force 重新申请，可能触发 CA 频率限制。",
                        "condition": {"dashboard.ssl.auto_acme.enable": True},
                    },
                    "log_file_enable": {
                        "description": "启用文件日志",
