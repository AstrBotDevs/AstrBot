...
class UmopConfigRouter:
    ...
    async def rename_platform_id(
        self, old_platform_id: str, new_platform_id: str
    ) -> int:
        """Rename the platform_id segment in existing UMO routing keys."""
        if old_platform_id == new_platform_id:
            return 0

        updated_routing: dict[str, str] = {}
        changed_count = 0
        for umo, conf_id in self.umop_to_conf_id.items():
            parts = self._split_umo(umo)
            if parts is None or parts[0] != old_platform_id:
                updated_routing[umo] = conf_id
                continue

            _, message_type, session_id = parts
            updated_routing[f"{new_platform_id}:{message_type}:{session_id}"] = conf_id
            changed_count += 1

        if changed_count:
            self.umop_to_conf_id = updated_routing
            await self.sp.global_put("umop_config_routing", self.umop_to_conf_id)
