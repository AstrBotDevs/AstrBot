...
class FunctionToolManager:
    ...
    async def init_mcp_clients(
        ...
        *,
        ...
        if not mcp_json_file.exists():
            # 配置文件不存在错误处理
            mcp_json_file.parent.mkdir(parents=True, exist_ok=True)
        ...
        mcp_server_json_obj: dict[str, dict] = self.load_mcp_config()["mcpServers"]
    ...
    async def _shutdown_runtimes(
        ...
        *,
        ...
        try:
            ...
            if strict:
                raise MCPShutdownTimeoutError(pending_names, timeout) from None
            logger.warning(
    ...
    def load_mcp_config(self):
        if not os.path.exists(self.mcp_config_path):
            ...
            with open(self.mcp_config_path, "w", encoding="utf-8") as f:
                json.dump(
                    normalize_mcp_config(DEFAULT_MCP_CONFIG),
                    f,
                    ensure_ascii=False,
                    indent=4,
                )
    ...
    async def list_mcp_servers_from_provider(
        ...
        return await get_mcp_sync_provider(provider).list_servers(payload)
    ...
    async def sync_mcp_servers_from_provider(
        ...
        items = await get_mcp_sync_provider(provider).fetch(payload)
        ...
        synced: list[SyncedMcpServer] = []
        for item in items:
            mcp_servers[item.name] = item.config
            synced.append(item)

        if not self.save_mcp_config(config):
            raise RuntimeError("Failed to save MCP configuration")

        tasks = [
            self.enable_mcp_server(name=item.name, config=item.config)
            for item in synced
            if item.config.get("active", True)
        ]
        if tasks:
        ...
        return {
            "provider": provider,
            "count": len(synced),
            "servers": [{"name": item.name, "config": item.config} for item in synced],
        }
