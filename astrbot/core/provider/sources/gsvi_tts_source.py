...
class ProviderGSVITTS(TTSProvider):
    ...
    async def get_audio(self, text: str) -> str:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            if not self.character:
                logger.warning(
                    "[GSVI TTS] character is not configured; falling back to legacy /tts",
                )
                await self._download_legacy_tts(session, text, str(path))
                return str(path)
            ...
            await self._download_binary(session, audio_url, str(path))
    ...
    async def _resolve_infer_config(
        self,
        session: aiohttp.ClientSession,
    ) -> dict[str, str]:
        versions = []
        ...
        for version in versions:
            model_data = await self._get_json(
                session, f"{self.api_base}/models/{version}"
            )
    ...
    def _select_prompt_text_lang(self, language_map: dict[str, list[str]]) -> str:
        ...
        if self.prompt_text_lang in language_map:
            return self.prompt_text_lang
        non_empty_languages = [
            lang for lang, emotions in language_map.items() if emotions
        ]
    ...
    async def _request_infer_audio(
        ...
        if not audio_url:
            raise Exception(
                data.get("msg", "GSVI TTS infer_single did not return audio_url")
            )
        return audio_url
    ...
    async def _download_binary(
        ...
        async with session.get(url) as response:
            if response.status != 200:
                error_text = await response.text()
                raise Exception(
                    f"GSVI TTS API 下载音频失败，状态码: {response.status}，错误: {error_text}",
                )
