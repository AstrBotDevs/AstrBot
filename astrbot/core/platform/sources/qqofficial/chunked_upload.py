...
class ApiError(Exception):
    """API 错误"""
    def __init__(self, message: str, status: int, path: str, biz_code: Optional[int] = None):
        self.status = status
        self.path = path
...
class ChunkedUploadError(Exception):
    """分片上传错误"""
    def __init__(self, message: str, file_path: str, file_size: int, cause: Optional[Exception] = None):
        self.file_path = file_path
        self.file_size = file_size
        self.cause = cause
        super().__init__(message)
...
class UploadCache:
    ...
    def __init__(self, max_size: int = 100):
        self._cache: Dict[str, Tuple[str, str, float]] = {}  # hash -> (file_info, file_uuid, expires_at)
        self._max_size = max_size
    
    def get(self, content_hash: str, target_type: str, target_id: str, file_type: int) -> Optional[str]:
        """获取缓存的 file_info"""
        key = f"{content_hash}:{target_type}:{target_id}:{file_type}"
        entry = self._cache.get(key)
        if entry:
            file_info, file_uuid, expires_at = entry
            if time.time() < expires_at:
                logger.debug(f"[UploadCache] Cache hit: {key[:16]}...")
                return file_info
            else:
                # 过期，删除
                del self._cache[key]
    ...
    def set(self, content_hash: str, target_type: str, target_id: str, file_type: int, 
           file_info: str, file_uuid: str, ttl: int) -> None:
        """缓存 file_info"""
        if len(self._cache) >= self._max_size:
            # 清理过期条目
            now = time.time()
            expired_keys = [k for k, (_, _, exp) in self._cache.items() if exp < now]
            for k in expired_keys:
                del self._cache[k]
        
        key = f"{content_hash}:{target_type}:{target_id}:{file_type}"
        expires_at = time.time() + ttl
        self._cache[key] = (file_info, file_uuid, expires_at)
        logger.debug(f"[UploadCache] Cached: {key[:16]}... (ttl={ttl}s)")
    ...
    async def get_client(cls, appid: str, secret: str) -> QQBotHttpClient:
        ...
        with cls._lock:
            if appid not in cls._clients:
                logger.debug(f"[QQBotHttpClientManager] Creating new client for appId={appid[:8]}...")
        ...
        with cls._lock:
            return {
                appid: {
                    ...
                    "token_expires_in": max(0, int(client._token_expires_at - time.time())) if client._token_expires_at else None
                }
    ...
    with open(file_path, 'rb') as f:
        while True:
            ...
            if need_10m:
                ...
                if remaining > 0:
                    md5_10m_hash.update(chunk[:remaining] if len(chunk) > remaining else chunk)
...
class QQBotHttpClient:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if "access_token" not in data:
                    ...
                    logger.error(f"[QQBotHttpClient:{self.appid}] Token fetch failed: {error_msg}")
                ...
                logger.debug(f"[QQBotHttpClient:{self.appid}] Token cached, expires in {expires_in}s")
                return self._token
    ...
    async def api_request(
        ...
        async with session.request(
            method, url, json=body, headers=headers, timeout=aiohttp.ClientTimeout(total=timeout)
        ) as resp:
                # 打印响应信息
                trace_id = resp.headers.get("x-tps-trace-id", "")
                logger.debug(f"[QQBotHttpClient] <<< Status: {resp.status} {resp.reason}" + (f" | TraceId: {trace_id}" if trace_id else ""))
                ...
                if not resp.ok:
                    try:
                        ...
                        logger.error(f"[QQBotHttpClient] API Error [{path}]: {error_msg} (bizCode={biz_code})")
                        raise ApiError(
                            f"API Error [{path}]: {error_msg}",
                            resp.status, path, biz_code
                        )
                    except Exception as e:
                        if isinstance(e, ApiError):
                            raise
                        logger.error(f"[QQBotHttpClient] API Error [{path}] HTTP {resp.status}: {raw[:200]}")
                        raise ApiError(f"API Error [{path}] HTTP {resp.status}: {raw[:200]}", resp.status, path)
    ...
    async def base64_upload(
        ...
        logger.info(f"[QQBotHttpClient] Base64 upload: target={target_type}:{target_id[:16]}, file_type={file_type}, size={len(file_data)} chars")
        
        data = await self.api_request(
            "POST", path, body=payload, timeout=300.0
        )
    ...
    async def c2c_upload_prepare(
        self, user_id: str, file_type: int, file_name: str, 
        file_size: int, hashes: UploadPrepareHashes
    ) -> UploadPrepareResponse:
        """C2C 申请上传"""
        logger.info(f"[QQBotHttpClient] C2C upload_prepare: user={user_id[:16]}, file={file_name}, size={file_size}")
        
        data = await self.api_request(
            "POST", f"/v2/users/{user_id}/upload_prepare",
            {
                "file_type": file_type,
                "file_name": file_name,
                "file_size": file_size,
        ...
        logger.info(f"[QQBotHttpClient] C2C upload_prepare success: upload_id={data['upload_id']}, parts={len(data['parts'])}")
        
        return UploadPrepareResponse(
            upload_id=data["upload_id"],
            block_size=int(data["block_size"]),
            parts=[UploadPart(index=p["index"], presigned_url=p["presigned_url"]) for p in data["parts"]],
            concurrency=int(data.get("concurrency", 1)),
            retry_timeout=int(data.get("retry_timeout", 0))
        )
    
    async def c2c_upload_part_finish(
        self, user_id: str, upload_id: str, part_index: int,
        block_size: int, md5: str, retry_timeout_ms: Optional[int] = None
    ) -> None:
        """C2C 完成分片上传（带持续重试）"""
        logger.debug(f"[QQBotHttpClient] C2C upload_part_finish: part={part_index}")
        await self._part_finish_with_retry(
            "POST", f"/v2/users/{user_id}/upload_part_finish",
            {
                "upload_id": upload_id,
    ...
    async def c2c_complete_upload(self, user_id: str, upload_id: str) -> MediaUploadResponse:
        """C2C 完成文件上传（带重试）"""
        result = await self._complete_upload_with_retry(
            "POST", f"/v2/users/{user_id}/files",
            {"upload_id": upload_id}
        )
        logger.info(f"[QQBotHttpClient] c2c complete_upload success: upload_id={upload_id}, file_uuid={result.file_uuid}")
        return result
    
    async def group_upload_prepare(
        self, group_id: str, file_type: int, file_name: str,
        file_size: int, hashes: UploadPrepareHashes
    ) -> UploadPrepareResponse:
        """Group 申请上传"""
        logger.info(f"[QQBotHttpClient] Group upload_prepare: group={group_id[:16]}, file={file_name}, size={file_size}")
        
        data = await self.api_request(
            "POST", f"/v2/groups/{group_id}/upload_prepare",
            {
                "file_type": file_type,
                "file_name": file_name,
                "file_size": file_size,
        ...
        logger.info(f"[QQBotHttpClient] Group upload_prepare success: upload_id={data['upload_id']}, parts={len(data['parts'])}")
    ...
    async def group_upload_part_finish(
        self, group_id: str, upload_id: str, part_index: int,
        block_size: int, md5: str, retry_timeout_ms: Optional[int] = None
    ) -> None:
        """Group 完成分片上传（带持续重试）"""
        await self._part_finish_with_retry(
            "POST", f"/v2/groups/{group_id}/upload_part_finish",
            {
                "upload_id": upload_id,
                "part_index": part_index,
                "block_size": block_size,
    ...
    async def group_complete_upload(self, group_id: str, upload_id: str) -> MediaUploadResponse:
        """Group 完成文件上传（带重试）"""
        return await self._complete_upload_with_retry(
            "POST", f"/v2/groups/{group_id}/files",
            {"upload_id": upload_id}
        )
    ...
    async def _part_finish_with_retry(
        ...
        for attempt in range(PART_FINISH_MAX_RETRIES + 1):
            try:
                ...
                # 命中特定错误码 → 进入持续重试模式
                if isinstance(err, ApiError) and err.biz_code in PART_FINISH_RETRYABLE_CODES:
                    timeout_ms = retry_timeout_ms or PART_FINISH_RETRYABLE_DEFAULT_TIMEOUT_MS
                    logger.warning(f"[chunked] PartFinish hit retryable bizCode={err.biz_code}, entering persistent retry (timeout={timeout_ms/1000}s)")
                    await self._part_finish_persistent_retry(method, path, body, timeout_ms)
                    return

                if attempt < PART_FINISH_MAX_RETRIES:
                    delay = PART_FINISH_BASE_DELAY_MS * (2 ** attempt) / 1000
                    logger.warning(f"[chunked] PartFinish attempt {attempt + 1} failed, retrying in {delay}s: {str(err)[:200]}")
    ...
    async def _part_finish_persistent_retry(
        ...
        while time.time() < deadline:
            try:
                await self.api_request(method, path, body, timeout=60.0)
                logger.info(f"[chunked] PartFinish persistent retry succeeded after {attempt} retries")
                return
            except Exception as err:
                # 如果不再是可重试的错误码，直接抛出
                if not (isinstance(err, ApiError) and err.biz_code in PART_FINISH_RETRYABLE_CODES):
                    logger.error(f"[chunked] PartFinish persistent retry: error is no longer retryable")
                    raise
                ...
                logger.warning(f"[chunked] PartFinish persistent retry #{attempt}: bizCode={err.biz_code}, retrying (remaining={int(remaining)}s)")
                await asyncio.sleep(PART_FINISH_RETRYABLE_INTERVAL_MS / 1000)

        raise RuntimeError(f"upload_part_finish 持续重试超时（{timeout_ms/1000}s, {attempt} 次重试）")
        ...
        for attempt in range(COMPLETE_UPLOAD_MAX_RETRIES + 1):
            try:
                ...
                if attempt < COMPLETE_UPLOAD_MAX_RETRIES:
                    ...
                    logger.warning(f"[chunked] CompleteUpload attempt {attempt + 1} failed, retrying in {delay}s")
                    await asyncio.sleep(delay)
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.put(
                    ...
                    if not resp.ok:
                        ...
                        raise RuntimeError(f"COS PUT failed: {resp.status} {body[:200]}")
                    
                    logger.debug(f"{prefix} Part {part_index}/{total_parts}: uploaded {len(data)} bytes")
                    return
        except Exception as e:
            last_error = e
            if attempt < PART_UPLOAD_MAX_RETRIES:
                delay = 1000 * (2 ** attempt) / 1000  # 1s, 2s
                logger.warning(f"{prefix} Part {part_index}/{total_parts}: attempt {attempt + 1} failed, retrying in {delay}s: {str(e)[:100]}")
                await asyncio.sleep(delay)
...
async def chunked_upload_c2c(
    ...
    logger.info(f"{prefix} Upload prepared: upload_id={upload_id}, block_size={block_size}, parts={len(parts)}, concurrency={concurrency}")
    ...
    async def upload_part(part: UploadPart) -> None:
        ...
        logger.debug(f"{prefix} Part {part_index}/{len(parts)}: uploading {length} bytes")
...
async def chunked_upload_group(
    ...
    logger.info(f"{prefix} Starting chunked upload (group): file={file_name}, size={file_size}, type={file_type}")
    ...
    logger.info(f"{prefix} Upload prepared: upload_id={upload_id}, block_size={block_size}, parts={len(parts)}")
    ...
    async def upload_part(part: UploadPart) -> None:
        ...
        await put_to_presigned_url(part.presigned_url, part_data, prefix, part_index, len(parts))
        await http_client.group_upload_part_finish(group_id, upload_id, part_index, length, part_md5, retry_timeout_ms)
