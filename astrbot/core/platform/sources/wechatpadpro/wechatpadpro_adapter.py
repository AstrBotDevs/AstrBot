...
class WeChatPadProAdapter(Platform):
    ...
    async def _download_raw_image(
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            while total_len is None or start_pos < total_len:
                ...
                data_json = await self._retry_request(session, url, params, payload)
                if not data_json:
                    return None
    ...
    async def _process_message_content(
        ...
        if msg_type == 1:  # 文本消息
            ...
            # )
            # image_bs64_data = (
            #     image_resp.get("Data", {}).get("Data", {}).get("Buffer", "")
            # )
            ...
            # image_bs64_data = raw_message.get("img_buf", {}).get("buffer", "")
            if image_bs64_data:
