...
def _replace_invalid_platform_id_char(match: re.Match[str]) -> str:
    return "" if match.group(0).isspace() else "_"
...
class PlatformManager:
    ...
    def _sanitize_platform_id(self, platform_id: str | None) -> tuple[str | None, bool]:
        ...
        sanitized = INVALID_PLATFORM_ID_PATTERN.sub(
            _replace_invalid_platform_id_char,
            platform_id,
        )
        ...
        try:
            ...
            if not self._is_valid_platform_id(platform_id):
                ...
                if sanitized_id and changed:
                    ...
                    self.astrbot_config.save_config()
                    if self.umop_config_router:
                        await self.umop_config_router.rename_platform_id(
                            old_platform_id,
                            sanitized_id,
                        )
                else:
