...
- 临时提示词渲染
- 对话历史提示词（带日期时间）
- 灵活提示词组装器（build_prompt_sections）
"""
...
### 长时间等待提示
你已经连续等待多次，且对话没有进展。
**如果确实没有更多可以说的，可以考虑使用 end_conversation 动作结束对话。**
如果用户长时间没有回复，也可以结束对话等待下次用户主动发起。
"""
...
def build_main_thinking_prompt(
    ...
    bot_name = robot_config.get("nickname") or persona.get("name", "助手")
    ...
    return MAIN_THINKING_SYSTEM_PROMPT.format(
        bot_name=bot_name,
        system_prompt=system_prompt or "你是一个助手",
...
def build_prompt_sections(
    *sections: str,
    ...
    将传入的多个提示词段落用分隔符连接起来，自动过滤空段落。

    Args:
        *sections: 可变数量的提示词段落
        separator: 段落之间的分隔符，默认 "\n\n---\n\n"
