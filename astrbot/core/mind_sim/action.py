...
class Action(ABC):
    ...
    def add_temp_prompt(
        self, content: str, rounds: int = 5, min_duration: float = 30.0
    ) -> None:
        """添加临时提示词（动作运行中调用）
...
class ActionExecutor:
    ...
    async def cleanup_completed(self) -> list[str]:
        ...
        Returns:
            被清理的 instance_id 列表
        """
        to_remove = [iid for iid, r in self._running.items() if r.action.is_done()]
        ...
        for tp in self._temp_prompts:
            ...
            if should_keep:
                ...
                if elapsed_int < 60:
                    ...
                    time_str = (
                        f"{elapsed_int // 3600}小时{(elapsed_int % 3600) // 60}分"
                    )
