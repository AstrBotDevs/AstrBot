...
class MindEvent:
    ...
    @classmethod
    def action_output(cls, action_name: str, output_type: str, content: str, metadata: dict | None = None) -> "MindEvent":
        """动作产出事件（reply/typing/error 等）"""
        return cls(
            type=MindEventType.ACTION_OUTPUT,
            data={"action": action_name, "output_type": output_type, "content": content, **(metadata or {})}
        )
    ...
    def trigger_think(cls, reason: str = "") -> "MindEvent":
        """触发主思考事件（动作完成后请求再次思考）"""
        return cls(
            type=MindEventType.TRIGGER_THINK,
            data={"reason": reason}
        )
