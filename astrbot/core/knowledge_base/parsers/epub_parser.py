...
_TOC_HEAD_RE = re.compile(r"^\s{0,3}(?:#{1,6}\s*)?(?:table of contents|contents|toc|目录|目次|もくじ)\s*$", re.I)
...
_GENERIC_ALT_RE = re.compile(r"^(?:image|img|picture|photo|illustration|figure|fig|cover|插图|图片|图像|封面)\s*[\d._-]*$", re.I)
...
def _is_internal(href: str) -> bool:
    ...
    return href.startswith("#") or href.endswith(".html") or href.endswith(".xhtml") or ".html#" in href or ".xhtml#" in href
