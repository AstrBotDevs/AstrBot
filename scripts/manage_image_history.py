...
def _logical_digest(connection: sqlite3.Connection) -> str:
    ...
    for name, sql in tables:
        ...
        if name.startswith("sqlite_") or sql is None:
            continue
        try:
                ...
                for item in content:
                    ...
                    for part in (
                        ...
                        if (
                            ...
                            and part.get("type") == "image_media_ref"
                        ):
                            ref = ImageMediaStore(media).read
                            # Constructing and reading the reference validates both files and hashes.
