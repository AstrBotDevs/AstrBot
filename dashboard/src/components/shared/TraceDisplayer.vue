...
<template>
  <div class="trace-wrapper">
    <div class="trace-table" ref="scrollEl" :style="{ height: tableHeight }">
      ...
      <!-- ========== 主 Agent 追踪表（完全保持原版） ========== -->
      <div class="section-divider main-divider">
        <span class="section-label main-label">Main Agent Traces</span>
        <span class="section-count main-count">{{ mainEvents.length }}</span>
      </div>
      ...
      <div class="trace-group" :class="{ highlight: highlightMap[event.span_id] }" v-for="event in mainEvents"
        :key="event.span_id">
      ...
      <!-- ========== SubAgent 分隔线 ========== -->
      <template v-if="subagentEvents.length > 0">
      <div class="section-divider">
        <span class="section-label">SubAgent Traces</span>
        <span class="section-count">{{ subagentEvents.length }}</span>
      </div>

      <!-- ========== SubAgent 追踪表（与原版样式一致） ========== -->
      <div class="trace-row trace-header">
        <div class="trace-cell span" style="grid-column: span 1;">Parent</div>
        <div class="trace-cell time">Time</div>
        <div class="trace-cell span">Event ID</div>
        <div class="trace-cell umo">UMO</div>
        <div class="trace-cell sender">Sender</div>
      ...
      <div class="trace-group subagent-group" :class="{ highlight: highlightMap[event.span_id] }" v-for="event in subagentEvents"
        :key="event.span_id">
        <div class="trace-row trace-event">
          <div class="trace-cell span" style="grid-column: span 1;">
            <div class="parent-ref" :title="event.parent_span_id">
              ↳ {{ getParentLabel(event.parent_span_id) }}
            </div>
          </div>
          <div class="trace-cell time">{{ formatTime(event.first_time) }}</div>
          <div class="trace-cell span" :title="event.span_id">
...
export default {
  ...
  computed: {
    mainEvents() {
      return this.events.filter(e => !e.parent_span_id);
    },
    subagentEvents() {
      return this.events.filter(e => !!e.parent_span_id);
    }
  ...
  methods: {
    ...
    processNewTraces(newTraces) {
      ...
      newTraces.forEach((trace) => {
        ...
        // 补全 parent_span_id
        if (trace.parent_span_id && !event.parent_span_id) {
      ...
      if (hasUpdate) {
        ...
        if (this.events.length > this.maxItems) {
          ...
          removed.forEach((event) => { delete this.eventIndex[event.span_id]; });
        }
        touched.forEach((spanId) => this.pulseEvent(spanId));
      }
    },
    getParentLabel(parentSpanId) {
      if (!parentSpanId) return '—';
      return parentSpanId.slice(0, 8);
    },
    toggleEvent(spanId) {
    ...
    pulseEvent(spanId) {
      if (!spanId) return;
      if (this.highlightTimers[spanId]) clearTimeout(this.highlightTimers[spanId]);
