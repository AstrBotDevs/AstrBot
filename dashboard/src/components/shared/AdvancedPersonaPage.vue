...
export default defineComponent({
  ...
  setup() {
        ...
        if (response.data.status === "ok" && response.data.data) {
          ...
          personaForm.value = {
            ...
            llm_model_config: data.llm_model_config || {
              ...
              image_caption_model:
                data.llm_model_config && data.llm_model_config.image_caption_model
                  ? data.llm_model_config.image_caption_model
                  : {
                      provider_id: "",
                      model: "",
                      temperature: 0.7,
    ...
    // 初始化
    onMounted(async () => {
      await Promise.all([loadTools(), loadSkills(), loadExistingPersonaIds(), loadProviders()]);
