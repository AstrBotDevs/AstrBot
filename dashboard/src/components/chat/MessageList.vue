<template>
  <div class="messages-container" ref="messageContainer" :class="{ 'is-dark': isDark }">
    ...
    <v-virtual-scroll ref="virtualScroll" :items="messages" :height="containerHeight"
      :item-height="dynamicEstimatedItemHeight" class="message-list" :class="{ 'loading-blur': isLoadingMessages }"
      @mouseup="handleTextSelection" @scroll="onScroll">
          ...
          <div v-if="msg.content.type == 'user'" class="user-message">
            <div class="message-bubble user-bubble" :class="{ 'has-audio': hasAudio(msg.content.message) }"
              ...
              <template v-for="(part, partIndex) in msg.content.message" :key="partIndex">
                ...
                <!-- 纯文本 -->
                <div v-else-if="part.type === 'plain' && part.text" style="
                    font-family: inherit;
                ...
                <div v-else-if="part.type === 'image' && part.embedded_url" class="image-attachments">
                  <div class="image-attachment">
                    <img :src="part.embedded_url" class="attached-image" @click="openImagePreview(part.embedded_url)" />
                  </div>
                </div>
                  ...
                  <div class="file-attachment">
                    <a v-if="part.embedded_file.url" :href="part.embedded_file.url"
                      :download="part.embedded_file.filename" class="file-link" :class="{ 'is-dark': isDark }" :style="isDark
                          ...
                          : {}
                        ">
                      <v-icon size="small" class="file-icon" :style="isDark ? { color: 'var(--v-theme-secondary)' } : {}
            ...
            <v-avatar class="user-avatar"> </v-avatar>
          </div>
          ...
          <div v-else class="bot-message">
            ...
            <div class="bot-message-content">
              ...
              <div class="message-actions" v-if="!msg.content.isLoading || index === messages.length - 1">
                ...
                <v-btn :icon="getCopyIcon(index)" size="x-small" variant="text" class="copy-message-btn" :class="{
...
export default {
  ...
  data() {
    return {
      ...
      // 动态高度计算相关
      messageHeights: new Map(), // 缓存已计算的消息高度
      heightObserver: null, // ResizeObserver实例
      isHeightCalculating: false, // 防止重复计算
      maxHeightCacheSize: 50, // 最大缓存数量
    };
  },
  computed: {
    // 动态计算的平均高度
    dynamicEstimatedItemHeight() {
      if (this.messageHeights.size === 0) {
        return this.estimatedItemHeight; // 默认180
      }
      ...
      for (const height of this.messageHeights.values()) {
        totalHeight += height;
        count++;
      }

      const averageHeight = Math.round(totalHeight / count);

      // 限制在合理范围内（100-500px）
      return Math.max(100, Math.min(500, averageHeight));
    },
  },
  async mounted() {
  ...
  methods: {
    ...
    // 动态高度计算方法
    getEstimatedItemHeight(msg) {
      if (!msg || !msg.content) {
        return 180; // 默认高度
      }

      let baseHeight = 120; // 基础高度

      if (msg.content.type === "user") {
        // 用户消息高度估算
        const messageParts = msg.content.message || [];
        for (const part of messageParts) {
          if (part.type === "image") {
            baseHeight += 200; // 图片附件
          } else if (part.type === "record") {
            baseHeight += 80; // 音频附件
          } else if (part.type === "plain" && part.text) {
            // 根据文本长度估算高度
            const textLength = part.text.length;
            const lines = Math.ceil(textLength / 60); // 每行约60字符
            baseHeight += lines * 24; // 每行约24px
          }
        }
      } else if (msg.content.type === "bot") {
        // Bot消息高度估算
        baseHeight = 200; // 基础高度更高
        ...
        for (const part of messageParts) {
          if (part.type === "reasoning") {
            ...
            baseHeight += 80 * (part.tool_calls?.length || 1); // 工具调用
          } else if (part.type === "plain" && part.text) {
            // 根据文本长度估算高度
            const textLength = part.text.length;
            const lines = Math.ceil(textLength / 80); // 每行约80字符
            baseHeight += lines * 24; // 每行约24px
      ...
      // 限制在合理范围内
      return Math.max(100, Math.min(500, baseHeight));
    },

    // 初始化高度观察器
    initHeightObserver() {
      if (!window.ResizeObserver) {
        return; // 浏览器不支持ResizeObserver
      }
      ...
      this.heightObserver = new ResizeObserver((entries) => {
        if (this.isHeightCalculating) {
          return; // 防止重复计算
        }

        this.isHeightCalculating = true;
        ...
        entries.forEach((entry) => {
          ...
          if (messageIndex !== null) {
            ...
            this.updateMessageHeightCache(parseInt(messageIndex), height);
          }
        });

        // 延迟重置标志，避免频繁计算
        setTimeout(() => {
          this.isHeightCalculating = false;
        }, 100);
      });
      ...
      this.$nextTick(() => {
        const messageElements = document.querySelectorAll(
          ".message-item[data-message-index]",
        );
    ...
    // 更新消息高度缓存
    updateMessageHeightCache(index, height) {
      this.messageHeights.set(index, height);

      // 限制缓存大小
      if (this.messageHeights.size > this.maxHeightCacheSize) {
        // 删除最旧的缓存项
        const oldestKey = this.messageHeights.keys().next().value;
        if (oldestKey !== undefined) {
          this.messageHeights.delete(oldestKey);
        }
      }
    },

    // 清理视界外元素
    cleanupOutOfViewElements() {
      // 这里可以添加逻辑来清理不在视图中的元素
      // 但v-virtual-scroll已经处理了虚拟化，所以这里主要是管理缓存

      // 如果缓存太大，清理一些
      if (this.messageHeights.size > this.maxHeightCacheSize * 2) {
        const keysToDelete = [];
        let count = 0;

        for (const key of this.messageHeights.keys()) {
          if (count >= this.maxHeightCacheSize) {
            keysToDelete.push(key);
          }
          count++;
        }

        keysToDelete.forEach((key) => this.messageHeights.delete(key));
      }
    },

    onScroll(scrollState) {
      // v-virtual-scroll 的 @scroll 事件传递的是滚动状态对象
      if (scrollState && typeof scrollState === "object") {
        ...
        // 检查是否滚动到顶部，触发加载更多消息
        if (scrollTop <= 50 && this.messages.length > 0) {
          console.log("Scrolled to top, triggering loadMore");
    ...
    handleTextSelection() {
      ...
      if (messageIndexAttr !== null) {
        ...
        const messageItems =
          messageContainer?.querySelectorAll(".message-item");
        if (messageItems) {
    ...
    scrollToMessage(messageId) {
      ...
      const messageItem = container.querySelector(
        `.message-item[data-message-index="${msgIndex}"]`,
      );
      if (messageItem) {
        // 如果消息在 DOM 中，直接滚动到它
        ...
        if (
          virtualScroll &&
          typeof virtualScroll.scrollToIndex === "function"
        ) {
          // 使用虚拟滚动组件的 scrollToIndex 方法
          virtualScroll.scrollToIndex(msgIndex);
        } else {
          // 备选方案：计算估计的滚动位置
          this.$nextTick(() => {
            // 等待下一帧，然后尝试再次查找元素
            const retryItem = container.querySelector(
              `.message-item[data-message-index="${msgIndex}"]`,
            );
            if (retryItem) {
    ...
    checkToolCallStatus() {
      ...
      for (const { content } of this.messages) {
        for (const part of content?.message ?? []) {
          ...
          for (const { finished_ts, ts } of part.tool_calls ?? []) {
            if (finished_ts === undefined) {
              // 找到运行中的工具调用
              hasRunning = true;

              // 检查是否是亚秒级（运行时间小于1秒）
              if (this.currentTime - ts < 1) {
                hasSubSecond = true;
              }
            }
          }
      ...
      return { hasRunning, hasSubSecond };
    },
