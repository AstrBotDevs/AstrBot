...
export function resolveToolDisplayName(toolName: string, availableTools: ToolDisplayNameSource[] = []): string {
...
export function matchesToolSearch(tool: ToolSearchSource, rawQuery: string): boolean {
  ...
  return [tool.display_name, tool.name, tool.description, tool.origin, tool.origin_name].some((value) =>
