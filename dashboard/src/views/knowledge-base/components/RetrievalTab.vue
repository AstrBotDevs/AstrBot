...
const performRetrieval = async () => {
  ...
  if (!Number.isInteger(topK.value) || topK.value <= 0 || topK.value > 100) {
    showSnackbar(t("validation.topKRange"), "warning");
    return;
  }
...
const openDocument = (result: any) => {
  ...
  router.push({
    ...
    params: { kbId: result.kb_id || props.kbId, docId: result.doc_id },
  });
};
</script>
