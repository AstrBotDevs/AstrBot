...
export default defineComponent({
  ...
  computed: {
    ...
    filteredUnselectedUmos(): string[] {
      ...
      return this.unselectedUmos.filter((u: string) => u.toLowerCase().includes(search));
    },
    ...
    filteredSelectedUmos(): string[] {
      ...
      return (this.editingGroup.umos || []).filter((u: string) => u.toLowerCase().includes(search));
    },
  },
    ...
    async loadUmos(): Promise<void> {
      ...
      try {
        ...
        if (resp.status === "ok") {
          ...
          this.availableUmos = resp.data.umos.filter((umo: string) => !existingUmos.has(umo));
        }
      } catch (error: unknown) {
        const e = asCatchError(error);
        this.showError(e.response?.data?.message || this.tm("messages.loadError"));
