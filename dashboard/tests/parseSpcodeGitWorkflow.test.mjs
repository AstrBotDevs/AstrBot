...
// 2026-06-24 回归测试:真实后端 `_make_envelope` 从不把 `success`
// 写入 `data`(spec §3:`success` 只用来决定 `reason` 的值)。
// 之前 parser 用 `asBoolean(d.success)` 读不到字段 → 永远 false →
// 把所有成功响应错误地走 failure 分支,snackbar 显示 "unknown error"。
// 下面 4 个测试锚定真实 wire shape:data 里没有 `success` 字段。
const stageRealShape = {
  reason: null,
  stderr: "",
  elapsed_ms: 281,
  umo: "webchat-1",
  ...
  directory: "C:/repo",
  staged: true,
  files: ["dashboard/src/components/chat/GitDiffSidebar.vue"],
  staged_count: 1,
};

test("parseSpcodeGitStage: success inferred from reason=null when data.success is absent (real backend wire shape)", () => {
  // 这是后端实际返回的形状:`data` 里只有 `staged` + `reason: null`,
  // 没有 `success` 字段。修复前 snap.success=false,前端显示 unknown error。
  const r = parseSpcodeGitStage({ status: "ok", data: { ...stageRealShape } });
  assert.equal(r.snapshot.success, true);
  assert.equal(r.snapshot.reason, null);
  assert.equal(r.snapshot.staged, true);
  assert.equal(r.snapshot.files[0], "dashboard/src/components/chat/GitDiffSidebar.vue");
});

test("parseSpcodeGitStage: failure inferred from reason=string when data.success is absent", () => {
  const r = parseSpcodeGitStage({
    status: "ok",
    data: {
      ...stageRealShape,
...
test("parseSpcodeGitCommit: success inferred from reason=null (real backend wire shape)", () => {
  const r = parseSpcodeGitCommit({
    status: "ok",
    data: {
      reason: null,
...
test("parseSpcodeGitLog: success inferred from reason=null (real backend wire shape)", () => {
  const r = parseSpcodeGitLog({
    status: "ok",
    data: {
      reason: null,
...
// 显式 `success` 字段仍需被尊重(spec 不允许但保持宽容)。
test("deriveSuccess: explicit data.success=true overrides reason=null absence", () => {
  // 极端情况:reason=null 但 success=false — 旧 spec 不会发生,但
  // 防御性写一个测试保证不会出现奇怪翻转。
  // (用 stageRealShape 调整 reason 字段,验证 parser 内部一致性。)
  const r1 = parseSpcodeGitStage({
    status: "ok",
    data: { ...stageRealShape, reason: "git_error", success: true },
  });
  // 显式 success=true 优先(防止将来某后端真写了 success 字段时
  // 误判)。但 reason 仍透传供 snackbar 展示。
  assert.equal(r1.snapshot.success, true);
  assert.equal(r1.snapshot.reason, "git_error");
});
