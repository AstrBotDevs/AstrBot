...
// ════════════════════════════════════════════════════════════════
// plugin v0.3+ schema: 去掉 value 字段后的兼容性验证
// ════════════════════════════════════════════════════════════════

test("validateInteractiveChoice: 无 value 字段(新 schema) → 通过", () => {
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
    options: [
  ...
  assert.equal(validateInteractiveChoice(part), true);
});

test("validateInteractiveChoice: 部分选项有 value,部分没有 → 仍通过", () => {
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
    options: [
      { id: "A", label: "选项 A", value: "选了 A" },
      { id: "B", label: "选项 B" },  // 无 value
    ],
  };
  assert.equal(validateInteractiveChoice(part), true);
});

test("validateInteractiveChoice: value 为 null → 通过(可选字段)", () => {
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
    options: [
      { id: "A", label: "选项 A", value: null },
      { id: "B", label: "选项 B" },
    ],
  };
  assert.equal(validateInteractiveChoice(part), true);
...
test("validateInteractiveChoice: value 为 number 非法类型 → 仍通过(可选)", () => {
  // 因为不再是必填,类型错误也被宽容
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
    options: [
      { id: "A", label: "选项 A", value: 42 },
      { id: "B", label: "选项 B" },
    ],
  };
  assert.equal(validateInteractiveChoice(part), true);
...
test("validateInteractiveChoice: 无 value 时 label 缺失 → 仍被拒", () => {
  // 防御性:确保我移除 value 检查时没破坏 label 检查
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
    options: [
  ...
  assert.equal(validateInteractiveChoice(part), false);
});

test("unwrapInteractiveChoice: 无 value 字段也能解包 + 校验通过", () => {
  // 镜像 plugin v0.3+ 真实输出
  const pluginJson = JSON.stringify({
    type: "interactive_choice",
    prompt: "请选择模型:",
    options: [
      { id: "A", label: "GPT-4", description: "更强但更慢" },
      { id: "B", label: "GPT-4 mini" },
    ],
  });
  const plainPart = { type: "plain", text: pluginJson };
  const unwrapped = unwrapInteractiveChoice(plainPart);
...
test("extractAskUserChoiceFromToolCall: tool result 无 value 字段,拆出成功", () => {
  // 镜像 SSE 实际形状(新 schema)
  const part = {
    type: "tool_call",
    tool_calls: [
      {
  ...
  const { extractedChoices } = extractAskUserChoiceFromToolCall(part);
  assert.equal(extractedChoices.length, 1);
  // 拆出的 choices.options 不应有 undefined value
  assert.equal(extractedChoices[0].options[0].value, undefined);
});

// ────────────────────────────────────────────────────────────
// getOptionSubmitText: 点击按钮后 emit 给 LLM 的文本
// ────────────────────────────────────────────────────────────

test("getOptionSubmitText: 有 value 时返回 value(老 schema 兼容)", () => {
  const opt = { id: "A", label: "选项 A", value: "选了 A" };
  assert.equal(getOptionSubmitText(opt), "选了 A");
});

test("getOptionSubmitText: 无 value 时返回 id + label 拼接(新 schema)", () => {
  const opt = { id: "A", label: "GPT-4" };
  assert.equal(getOptionSubmitText(opt), "A. GPT-4");
});

test("getOptionSubmitText: 无 value,含 description 也仍只拼 id+label", () => {
  const opt = { id: "B", label: "删除", description: "不可逆" };
  assert.equal(getOptionSubmitText(opt), "B. 删除");
});

test("getOptionSubmitText: value 是空字符串视为无 value,回退到 id+label", () => {
  const opt = { id: "A", label: "选项 A", value: "" };
  assert.equal(getOptionSubmitText(opt), "A. 选项 A");
});

test("getOptionSubmitText: id 和 label 都有,返回 'id. label'", () => {
  const opt = { id: "C", label: "本地模型" };
  assert.equal(getOptionSubmitText(opt), "C. 本地模型");
});

test("getOptionSubmitText: 缺 id(防御),仅返回 label", () => {
  const opt = { id: "", label: "孤 label" };
  assert.equal(getOptionSubmitText(opt), "孤 label");
});

test("getOptionSubmitText: 缺 label(防御),仅返回 id", () => {
  const opt = { id: "Z", label: "" };
  assert.equal(getOptionSubmitText(opt), "Z");
});

test("getOptionSubmitText: 端到端:InteractiveChoicePart + truncate + getOptionSubmitText", () => {
  // 模拟真实流程
  const part = {
    type: "interactive_choice",
    prompt: "选一个",
  ...
  assert.equal(validateInteractiveChoice(part), true);
  const truncated = truncateInteractiveChoice(part);
  // 选中第一项后 emit
  assert.equal(getOptionSubmitText(truncated.options[0]), "A. GPT-4");
  // 选中第三项
  assert.equal(getOptionSubmitText(truncated.options[2]), "C. 本地");
});