...
def test_find_missing_requirements_from_lines_tracks_missing_and_version_mismatches(
    monkeypatch,
):
    monkeypatch.setattr(
        requirements_utils,
    ...
    missing = requirements_utils.find_missing_requirements_from_lines(
        ["boto3>=2.0", "botocore"]
    )

    assert missing == {"boto3", "botocore"}
...
def test_iter_requirements_supports_direct_line_input():
    parsed = list(
        requirements_utils.iter_requirements(
            lines=["demo-package>=1.0", 'other-package; python_version < "0"']
        )
    )
