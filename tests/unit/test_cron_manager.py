...
class TestRunActiveAgentJob:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param({"plugin_set": []}, {"reserved"}, id="empty"),
            pytest.param(
                {"plugin_set": ["*"]},
            ...
            pytest.param(
                {}, {"allowed", "disabled", "reserved"}, id="default"
            ),
        ],
    )
    async def test_woke_main_agent_filters_plugin_hooks_and_tools(
        ...
        observed = {"tools": set(req.func_tool.names())}
        expected = {"tools": expected_plugins}
        for (name, hook_type), hook in hooks.items():
            observed[name, hook_type.name] = hook.await_count
            expected[name, hook_type.name] = int(name in expected_plugins)
