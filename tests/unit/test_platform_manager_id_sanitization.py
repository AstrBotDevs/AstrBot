...
class FakeSharedPreferences:
    ...
    async def global_put(self, key: str, value: dict[str, str]) -> None:
...
def test_empty_platform_id_is_invalid(manager: PlatformManager) -> None:
    ...
    assert manager._sanitize_platform_id("") == ("", False)
...
async def test_load_platform_sanitizes_whitespace_id(
    ...
    start_platform_task = MagicMock()
    monkeypatch.setattr(manager, "_start_platform_task", start_platform_task)
    manager.umop_config_router = MagicMock()
    manager.umop_config_router.rename_platform_id = AsyncMock()
    platform_config = {
        "enable": True,
        "type": "dummy",
        "id": "lark - 藤田琴音 Bot",
    ...
    manager.umop_config_router.rename_platform_id.assert_awaited_once_with(
        "lark - 藤田琴音 Bot",
        "lark-藤田琴音Bot",
    )
    task_name, inst = start_platform_task.call_args.args
    assert task_name == "platform_dummy_lark-藤田琴音Bot"
    assert inst.meta().id == "lark-藤田琴音Bot"


@pytest.mark.asyncio
async def test_umop_config_router_renames_platform_id_routes() -> None:
    sp = FakeSharedPreferences()
    router = UmopConfigRouter(sp)
    router.umop_to_conf_id = {
        "lark - 藤田琴音 Bot::": "platform-conf",
        "lark - 藤田琴音 Bot:GroupMessage:oc_xxx": "group-conf",
        "telegram:GroupMessage:oc_xxx": "other-conf",
    }

    changed_count = await router.rename_platform_id(
        "lark - 藤田琴音 Bot",
        "lark-藤田琴音Bot",
    )
    ...
    assert router.umop_to_conf_id == {
        "lark-藤田琴音Bot::": "platform-conf",
        "lark-藤田琴音Bot:GroupMessage:oc_xxx": "group-conf",
        "telegram:GroupMessage:oc_xxx": "other-conf",
    }
    assert sp.values["umop_config_routing"] == router.umop_to_conf_id
