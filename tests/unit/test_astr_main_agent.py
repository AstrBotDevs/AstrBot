...
class TestBuiltinToolInjection:
    ...
    @pytest.mark.asyncio
    async def test_apply_web_search_tools_adds_kagi_search_and_extract_tools(
        self, mock_event, mock_context
    ):
        """Test Kagi web search injects search and extract tools."""
        module = ama
        req = ProviderRequest()
        mock_context.get_config.return_value = {
            "provider_settings": {
                "web_search": True,
                "websearch_provider": "kagi",
            }
        }
        search_tool = MagicMock(spec=FunctionTool)
        search_tool.name = "web_search_kagi"
        extract_tool = MagicMock(spec=FunctionTool)
        extract_tool.name = "kagi_extract_web_page"
        tool_mgr = MagicMock()
        tool_mgr.get_builtin_tool.side_effect = [search_tool, extract_tool]
        mock_context.get_llm_tool_manager.return_value = tool_mgr
        await module._apply_web_search_tools(mock_event, req, mock_context)
        assert tool_mgr.get_builtin_tool.call_args_list == [
            ((module.KagiWebSearchTool,),),
            ((module.KagiExtractWebPageTool,),),
        ]
        assert req.func_tool is not None
        assert req.func_tool.get_tool("web_search_kagi") is search_tool
        assert req.func_tool.get_tool("kagi_extract_web_page") is extract_tool
